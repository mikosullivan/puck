...
**How the deepest frame is found.** Only frame 0 of a process carries `process_pk`; every sub-frame carries `parent_frame` pointing at the frame that pushed it. So the walk is:
...
2. Loop: `select object_pk from objects where primitive = 'f' and parent_frame = ?` — one indexed lookup per hop, taking `object_pk` from the previous step as the parent. Under the current push model there's at most one such child per frame.
...
local function get_latest_frame(db, process_pk)
	...
	-- 3. Walk down through `parent_frame` inverse — each hop finds
	...
	local child_stmt = db:prepare(
		"select object_pk from objects " ..
		"where primitive = 'f' and parent_frame = ?"
	)
