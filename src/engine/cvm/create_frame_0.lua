--[[
{
	...
	"role": "CVM routine: creates frame 0 for a fresh process. Composes `initialize_process` (one INSERT into `processes`, returns the new pk) with the frame INSERT (one INSERT into `objects` — `primitive='f'`, the CaspM in `ast`, the new process pk in `process`). Both writes wrap in a single SAVEPOINT / RELEASE SAVEPOINT with pcall + ROLLBACK TO SAVEPOINT on error, so the routine composes cleanly whether or not the caller has an outer transaction open.",
	"exports": {
