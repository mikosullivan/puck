...
{
	"module": "sequence",
	"role": "A monotonically incrementing string-integer counter. Every allocation in Drinian (object IDs, reference IDs, hash-element IDs, src keys, ast keys) draws from a shared Sequence stashed on the state hash. Encapsulates the counter's value + the increment algorithm so callers just say `:next()` at the allocation site.",
	"exports": {
...
[drinian/objects § Object IDs](https://www.puck.uno/requirements/drinian/objects#object-ids).
]]
...
NOT the Drinian state hash. State creation goes through `state.new()`
