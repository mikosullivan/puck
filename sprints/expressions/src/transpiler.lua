...
local function transpile_statement(stmt)
	...
	-- like `1(2)` or `1 + 2` fall through to the expression fallback.
	...
	-- Sprint-added: bare-identifier statements now fall through to the
	-- expression fallback at the end of transpile_statement. Previously a
	-- `bwc_bare = stmt:match("^([%w_]+)$")` pattern here matched a bare
	-- identifier or number and produced `{bwc: name}` — but that pattern
	-- also matched pure-digit tokens (`1`, `42`) and boolean-word tokens
	-- (`true`, `false`), classifying them as bareword commands instead
	-- of value literals. Removing the pattern lets parse_expression handle
	-- them uniformly via parse_literal (value literals → `{value: X}`) or
	-- its own bare-ident path (real bareword commands like `puts` → `{bwc: name}`).
	...
	-- Sprint-added: expression fallback. If no statement-shape pattern
	-- matched, try parsing the whole thing as an expression. If it
	-- parses, wrap the resulting atom as a one-atom statement row. Under
	-- the expressions-sprint evaluation model, every command produces an
	-- rv, and any parseable expression is a legal command (its rv is the
	-- expression's value).
	--
