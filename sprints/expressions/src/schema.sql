...
-- vibecode
/*
{"doc": "cvm-schema-sqlite", "version": "12.0-expressions-sprint",
	"sprint_context": "Fork of production schema 12.0 for the expressions sprint. Adds the `frames_child_delete_propagates_rv` trigger to propagate a reaping frame's rv (return value) to its parent's rv slot. All other content is byte-for-byte production 12.0.",
...
insert into cvm (key, value) values ('schema', '12.0-expressions-sprint');
...
-- Sprint-added trigger: propagates the reaping frame's rv to its parent.
-- Under the design where "every frame has a return value, even if
-- implicitly null," parent's rv after a child reap = child's rv at reap.
-- Uses the standard bucket-key mechanism: rv is stored as a ref from
-- the frame's bucket with key='rv' pointing at any object.
--
-- Fires on EVERY nested-frame reap — the hottest trigger on the schema
-- once nested-frame dispatch is common. The two-statement body is
-- optimized for the update-in-place hot path via ON CONFLICT DO UPDATE:
--
-- 1. UPSERT — if child has an rv, either update parent's existing rv
--    ref's child in place (hot path — no DELETE + INSERT churn) or
--    insert a new one if parent didn't have an rv yet (first-reap
--    case). Silent no-op via CROSS JOIN when child had no rv or no
--    bucket.
-- 2. DELETE — if child had no rv AND parent has an existing rv ref,
--    delete parent's rv ref (parent rv → implicitly null).
--
-- Exactly one of statement 1 or statement 2 does work per fire; the
-- other is a no-op. The hot path (both parent and child have rv) is
-- one UPDATE statement — no DELETE + INSERT cascade, and only one
-- needs_trace mark (via `refs_mark_needs_trace_after_child_update`).
--
-- Cap-exempt via the `old.frame_parent is not null` guard: the cap has
-- no parent, so cap deletion doesn't fire this trigger.
--
-- Walking-skeleton limitation: assumes parent's bucket exists. Under
-- current handlers, parent always has a bucket by the time a nested
-- frame could reap. Fully general behavior would materialize parent's
-- bucket on demand; deferred. [ghi]
create trigger frames_child_delete_propagates_rv
before delete on objects
when old.control = 'f'
	and old.frame_parent is not null
begin
	-- Statement 1a: materialize parent's bucket on demand. Only fires
	-- if both:
	--   - child has an rv (something to propagate), and
	--   - parent has no bucket yet.
	-- The new bucket inherits the parent frame's owner_role for FK
	-- compliance. object_pk gets a fresh UUID via the column's DEFAULT.
	--
	-- No point creating a bucket just to leave it empty — if the child
	-- had no rv (nothing to propagate), skip. Similarly if the parent
	-- already has a bucket, skip (statement 1b covers the link).
	...
	-- Statement 1b: link the new bucket to parent. Fires only if 1a
	-- actually inserted (changes() > 0 guard). last_insert_rowid()
	-- gives the rowid of the just-inserted objects row; we look up
	-- its object_pk (a UUID, not the rowid) to use as the ref child.
	...
	-- Statement 2: UPSERT — updates parent's existing rv ref's child
	-- in place, or inserts a new one if parent had no rv yet. The
	-- inner subquery packs the parent-bucket-pk and child-rv-value-pk
	-- into one row; the WHERE clause filters out cases where either
	-- is null (child had no rv, or parent had no bucket AND 1a didn't
	-- create one because there was nothing to propagate).
	--
	-- Wrapping the two lookups in a subquery (rather than joining
	-- directly in FROM) is a SQLite parser workaround — ON CONFLICT
	-- after a top-level JOIN triggers a "near 'do': syntax error"
	-- because the parser reads "on" as a JOIN condition. Nested
	-- SELECT avoids that.
	insert into refs (parent, child, key, idx)
	select
		pb,
	...
	-- Statement 3: DELETE — clears parent's existing rv ref when the
	-- child had no rv (child_rv subquery empty). The DELETE fires the
	-- standard refs_mark_needs_trace_after_delete on the old value.
	delete from refs
	where key = 'rv'
	  and parent in (
		select r.child from refs r
