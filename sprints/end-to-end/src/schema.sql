...
--
-- **code change**
-- Immutability decision on `ast` is being flipped from mutable to
...
-- **code change**
-- The following two triggers are being added. They validate that any
-- frame's `ast` value is (a) parseable JSON and (b) a JSON array. The
-- CHECK constraint above already enforces "not null when primitive =
-- 'f'"; these triggers extend that with structural validation.
--
-- Two separate triggers rather than one:
--   - INSERT trigger fires on every new frame row.
--   - UPDATE trigger fires only when ast is being modified (BEFORE
--     UPDATE OF ast), so unrelated column updates on frame rows
--     (advancing stmt_idx, setting complete, etc.) don't re-validate.
-- Both use the same WHEN guard (`new.primitive = 'f'`) and the same
-- CASE body — SQLite has no trigger inheritance, so the body is
-- duplicated verbatim. [ghi]
...
-- **code change**
-- Replaces what was `objects_ast_valid_update`. Under the design's
-- ast-is-immutable decision, there's no such thing as a valid
-- UPDATE to ast — any change gets rejected outright. Insert-time
-- validation still lives in `objects_ast_valid_insert` above.
...
-- **code change**
-- The trigger below is being added.
--
-- Advancing a frame's stmt_idx means "done with the current statement,
-- moving to the next." Per the sprint's invariant, a frame with
-- children is mid-execution; once the parent moves past that statement,
-- any children pushed by it are stale and must go. Bundling the child
-- delete into the same UPDATE as the stmt_idx bump makes it atomic at
-- the SQL layer — no savepoint dance needed on the Lua side, and no
-- way to advance stmt_idx without clearing children.
--
-- WHEN gate: `new.primitive = 'f'` restricts firing to frames.
-- Non-frame rows can't have stmt_idx updated anyway (CHECK constraint
-- on the column), but the guard is cheap and explicit.
...
-- The trigger below is being added.
--
-- Deleting a frame whose `process` column is set (frame 0 of a process)
-- means that process is done. Flip `processes.complete = 1` from within
-- this trigger so the mark-complete + delete-frame pair is one atomic
-- SQL operation — no Lua-side savepoint needed, and no way to delete a
-- frame-0 row without marking its process complete.
--
-- WHEN gate: `old.primitive = 'f' AND old.process IS NOT NULL` restricts
-- firing to frame-0 rows specifically. Sub-frames (`process` null) fall
-- through — they're deleted routinely by the child-cleanup trigger
-- above without needing to affect any process's state.
...
create table processes (
	...
	-- **code change**
	-- The following two columns (complete, message) are being added.
	...
	-- without null-checks. Set to 1 when the walker exhausts frame 0's
	-- ast and pops the last frame off the stack. [ghi]
	complete integer not null default 0 check (complete in (0, 1)),

	-- Message. Text
	-- The value of this field is returned after the finished run. [ghi]
	message text default null
);

-- **code change**
-- The trigger below is being modified. In shipping it raises on ANY
-- update to a processes row. Here we allow updates to columns other
-- than `process_pk` so the walker can set `complete = 1` (and
-- eventually `message = <value>`) at shutdown. Only `process_pk`
-- remains immutable.
