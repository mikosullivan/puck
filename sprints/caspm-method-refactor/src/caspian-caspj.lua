--[[
{
	"module":  "caspian-caspj",
	"role":    "Caspian source -> CaspJ. Sprint copy renamed from `transpiler` — production's copy at production/src/engine/transpiler.lua stays as-is until the sprint lands. Standalone module; no knowledge of the engine or of the CaspJ -> CaspM step (see caspj-caspm).",
	"exports": {
		"transpile": "(source: string, opts?: {lines: bool}) -> CaspJ. opts.lines = true opt-in mode: every statement row gets a trailing `{line: N}` meta-atom, and every value-atom object gets a `line: N` field. Default: no line annotations.",
		"null":      "sentinel for JSON null in CaspJ output (matches dkjson.null so test assertions align)"
	}
}
]]
...
First half of the Caspian pipeline: turns Caspian source into the
self-documenting "full" CaspJ tree that `caspj-caspm.lua` compacts
into CaspM before the engine walks it. Standalone module — no
knowledge of the engine, no shared state with the runtime, no
round-trip contract beyond the CaspJ shape.
...
parse_expression = function(s)
	...
	-- Numeric-literal receiver: `10.times`, `3.14.round`, `0xFF.abs`, etc.
	-- Same tail-must-start-with-chain rule as the bareword case above.
	-- Prefer longer numeric forms first so `3.14.round` matches `3.14` as a
	-- float rather than `3` as an int followed by `.14.round`. Leading `-`
	-- is deliberately NOT consumed here — unary-minus precedence vs `.method`
	-- is unsettled, and letting `-10.abs` fall through to the general
	-- expression parser keeps the current behaviour (rather than committing
	-- to one reading via the numeric-receiver shortcut).
	if not ec_recv then
		local numeric_prefix, base

		local hex_prefix = s:match("^(0x[0-9A-Fa-f_]+)")

		if hex_prefix then
			numeric_prefix, base = hex_prefix, "hex"
		else
			local oct_prefix = s:match("^(0o[0-7_]+)")

			if oct_prefix then
				numeric_prefix, base = oct_prefix, "oct"
			else
				local bin_prefix = s:match("^(0b[01_]+)")

				if bin_prefix then
					numeric_prefix, base = bin_prefix, "bin"
				else
					-- Float (digits, dot, digits) preferred over int so
					-- `3.14.round` reads as `3.14` then `.round`.
					numeric_prefix = s:match("^(%d[%d_]*%.%d[%d_]*)")
						or s:match("^(%d[%d_]*)")
				end
			end
		end
		...
		if numeric_prefix then
			local rest = s:sub(#numeric_prefix + 1)
			local first = rest:sub(1, 1)

			if first == "." or first == "[" then
				local value = parse_number(numeric_prefix)

				if value ~= nil then
					local atom = {value = value}
					if base then atom.base = base end
					ec_recv = attach_line(atom)
					ec_after = rest
				end
			end
		end
