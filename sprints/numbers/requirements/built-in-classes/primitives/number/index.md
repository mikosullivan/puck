...
The one place `-0` differs from `0` is **[array indexing](https://puck.uno/requirements/built-in-classes/primitives/array/#zero-based-indexing-from-both-ends)**: `$arr[-0]` is the last element, while `$arr[0]` is the first. Every other consumer of the value — equality, hash keys, JSON serialization, comparison, `.to_string`, `.to_hex`, `.to_bin` — treats `-0` as `0`.
...
Boolean-returning methods for querying a value (return a [boolean](https://puck.uno/requirements/built-in-classes/primitives/boolean)):
...
Numeric iteration helpers — `.times`, `.upto`, `.downto` — live under [loops § Numeric helpers](https://puck.uno/requirements/syntax/loops#numeric-helpers-times-upto-downto).
