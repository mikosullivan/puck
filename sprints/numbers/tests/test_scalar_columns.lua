...
local SCHEMA_PATH    = 'sprints/numbers/src/engine/cvm/sqlite/schema.sql'
...
-- ------------------------------------------------------------
-- Standalone report tail. When invoked via production's run.lua the
-- shared harness prints its own aggregate; when invoked directly on
-- this file (`lua5.4 sprints/numbers/tests/test_scalar_columns.lua`)
-- the same helpers.results accumulator is here for us to summarise.
-- ------------------------------------------------------------
