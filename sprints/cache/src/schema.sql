...
-- **code change**
-- The `user` column has been replaced with `core_role`. Where the old
-- column carried a single truthy marker (user = 1) for the one root
-- role, `core_role` is a one-character discriminator that identifies
-- which core role a row is: 'e' (engine), 'c' (core), 'u' (user).
-- Nullable — most rows aren't core roles. At most one row per value
-- (uniqueness enforced by the partial unique index below). [ghi]
alter table objects add column core_role text
...
-- **code change**
-- Core-role uniqueness: partial unique index enforces "at most one
-- row per core_role value" — at most one 'e', one 'c', one 'u'. The
-- `where core_role is not null` clause keeps the index empty on the
-- (many) null rows and holds at most three entries. [ghi]
...
-- **code change**
-- The first branch of the roles view now uses `core_role is not null`
-- (any core role — engine, core, or user) instead of `user = 1` (the
-- single old root marker).
create view roles as
	select object_pk from objects where core_role is not null
	union
...
-- **code change**
-- Renamed from `objects_user_immutable`. Core-role assignment is set
-- at INSERT (on each seeded core-role row) and never changes.
...
-- **code change**
-- Seeds three core-role rows instead of one: engine (root of the
-- core-role tree), then cache and user (both children of engine
-- via role_parent). All three are HashPrimitives (primitive = 'h')
-- and pinned (persistent = 1). Seeded before the ownership
-- triggers below fire, so their `owner_role = null` state is
-- grandfathered. The FK on role_parent still resolves — engine
-- exists at the moment cache and user get inserted because it's
-- inserted first. [ghi]
...
-- **code change**
-- The XOR clause was dropped. Under the cache-sprint design, roles
-- (rows with role_parent or core_role set) CAN also have owner_role
-- set — cache and user are both roles AND owned by engine, for
-- example. The remaining rule is asymmetric: non-role rows must have
-- owner_role set. Roles may or may not.
...
-- **code change**
-- Guard triggers on ANY core-role row (engine, core, or user), not
-- just the user seed. All core-role rows are foundational and can't
-- be deleted.
...
-- **code change**
-- Same shape as objects_no_delete_root_role: guard fires for any
-- core-role row (engine, core, or user), not just the user seed.
...
-- **code change**
-- Column renamed from `process` to `process_pk`. The unsuffixed-FK
-- naming convention (owner_role, frame_parent, role_parent, bucket_for,
-- stack_for) applies only to self-referencing FKs within `objects`;
-- FKs to a different table (here, `processes`) get the `_pk` suffix.
alter table objects add column process_pk text
