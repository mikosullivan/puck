--[[
{
  "module": "charlie.transpiler",
  "role": "Charlie AST → CharlieJSON — converts a parsed program node to a Lua table tree ready for JSON encoding",
  "exports": { "transpile": "AST program node → CharlieJSON table  [stmt, ...]" },
  "expression_forms": {
...
--[[ { "in": {"recv_ksj": "CharlieJSON expr", "method": "string", "pos_args": "node[]?", "kw_args": "{key,value}[]?", "block_node": "AST block?"}, "out": "call array  [recv, method]  or  [recv, method, args_obj]" } ]]
...
--[[ { "in": "node: AST block node or nil", "out": "CharlieJSON block object or nil", "note": "copies params; transpiles body, before, between, after, noloop; preserves name" } ]]
...
--[[ { "in": "node: pipe AST node", "out": "{'pipe': {stages:[CharlieJSON expr,...], null_safe:bool}}", "note": "walks the left-recursive pipe chain to collect stages left-to-right; any |& in the chain sets null_safe=true" } ]]
...
--[[
{
  ...
  "out": "CharlieJSON expression value (tagged object, call array, or JSON_NULL)",
...
--[[ { "in": "node: assign AST node", "out": "CharlieJSON statement", "dispatches_on": ["var→setvar", "ivar→setivar", "varobj→setvarobj", "index→[]=", "method_call→setter", "func_call→setfunc", "sys→setsys"] } ]]
...
--[[ { "in": "body: decl AST node[]", "out": "CharlieJSON decl object[]", "note": "maps each class-body declaration kind to its CharlieJSON decl object; unknown kinds silently skipped; recurses for helper bodies" } ]]
...
--[[ { "in": "node: AST statement node", "out": "CharlieJSON statement", "dispatches_on": ["assign", "expr_stmt", "return_stmt", "yield_stmt", "if_stmt", "while_stmt", "func_def", "class_def", "catch_stmt", "heed_stmt"] } ]]
...
--[[ { "in": "nodes: AST node[]", "out": "CharlieJSON statement[]", "note": "maps transpile_stmt over the array; used for bodies, branches, and class bodies" } ]]
...
--[[ { "in": "ast: program node  {kind:'program', stmts:[...]}", "out": "CharlieJSON Lua table  [stmt,...]", "note": "public entry point; errors if ast.kind ~= 'program'" } ]]
