--[[
{
  "module": "charlie.engine",
  "role": "V0.01 canonical-CharlieJSON executor — consumes the [receiver, method, args?] statement shape from charliejson.md",
  ...
  "exports": {
    ...
    "materialize":   "(expr) → value  turn a CharlieJSON expression into a tagged value table",
  ...
  "relationship_to_existing_interpreter": "engine.lua handles canonical CharlieJSON for V0.01; interpreter.lua handles the pre-canonical shape emitted by the existing transpiler. They coexist; no shared state.",
  "depends_on": ["charlie.json"],
  "docs": ["documentation/development/development.md (Rand sketch)", "documentation/charlie/charliejson.md", "documentation/charlie/roles.md"]
}
]]
...
--[[
{
  "in":  "expr: a CharlieJSON expression",
...
function M.run(path)
    ...
    if type(tree) ~= "table" then
        error("engine.run: top-level CharlieJSON must be an array of statements")
    end
