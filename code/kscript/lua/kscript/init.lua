...
{
  "module": "kscript",
  "role": "Public API — single entry point for the KScript toolchain",
  "pipeline": [
    "KScript source (.kscript file or string)",
    "lexer  →  token[]",
    "parser →  AST",
    "transpiler → KScriptJSON (Lua table)",
  ...
  "exports": {
    ...
    "transpile": "string → table             full pipeline to KScriptJSON as a Lua table (array of statements)",
    ...
    "null":      "sentinel                   JSON null value (distinct from Lua nil; use in KScriptJSON output)"
  },
  "depends_on": ["kscript.lexer", "kscript.parser", "kscript.transpiler", "kscript.json"],
  "docs": ["documentation/kscript/kscript.md", "documentation/kscript/kscriptjson.md"],
  "tests": "tests/kscript/run.lua"
}
]]
local lexer       = require("kscript.lexer")
local parser      = require("kscript.parser")
local transpiler  = require("kscript.transpiler")
local json        = require("kscript.json")
local interpreter = require("kscript.interpreter")

local M = {}

--[[ { "what": "JSON null sentinel", "why": "Lua nil cannot round-trip through tables; use M.null when building KScriptJSON that contains a null value" } ]]
M.null = json.null
...
--[[ { "in": {"source": "string"}, "out": "KScriptJSON table  [stmt,...]", "note": "full pipeline; statements are arrays for calls, tagged objects for expressions" } ]]
