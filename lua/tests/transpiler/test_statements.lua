...
--[[ { "in": "src: string", "out": "KScriptJSON table  (all statements)", "note": "transpile helper; returns ks.transpile(src)" } ]]
local function tx(src)
    return ks.transpile(src)
end

--[[ { "in": "src: string", "out": "KScriptJSON node  (first statement)", "note": "transpile helper; returns tx(src)[1]" } ]]
