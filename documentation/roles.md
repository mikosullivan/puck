...
vibecode: {
    "section": "overview",
    "role": "the official security model for KScript: every object owned by a role, code runs as the owning role of the function-object executing it, security boundaries are cross-role calls, faucets are the only way to pull objects in, jails are the explicit narrowing mechanism",
    ...
    "supersedes": ["trust.md_binary_trust_model", "%chain.trust", "%chain.allow_abort_escalation",
        "%chain.allow_catch_security_exceptions"]
}
```

The role model is KScript's security model. **Every object is owned by a
...
This supersedes the earlier binary trust/untrust model documented in
[trust.md](trust.md). That doc describes the previous design and is
preserved for historical reference; new code follows the role model
described here.
...
[kscript/kscript-runtime.md](kscript/kscript-runtime.md) covers this;
...
KScript code. The runtime bails to the engine immediately, and the engine
...
- Anything where the engine specifically wants to ensure no KScript code
...
When KScript pulls a value through a faucet, the runtime tags that value
...
- **Kiera.** See [kiera.md](kiera.md) for the full kiera model;
...
- This overlaps with
  [ideas/string-provenance.md](ideas/string-provenance.md). Worth
...
- Engine firewall rules — adapted to operate on roles instead of trust
  tags.
- [ideas/trusted-database-filtering.md](ideas/trusted-database-filtering.md)
...
- [trust.md](trust.md) — the previous binary-trust model, superseded by
  this doc. Preserved for historical reference.
- [kiera.md](kiera.md) — the kiera object model, which builds on role
...
- [ideas/string-provenance.md](ideas/string-provenance.md) — deferred
  idea for fine-grained string provenance.
- [ideas/trusted-database-filtering.md](ideas/trusted-database-filtering.md)
  — becomes more direct under per-source owning roles.
- [ideas/firewall.md](ideas/firewall.md) — engine firewall rules will
