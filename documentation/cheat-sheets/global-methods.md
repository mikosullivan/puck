...
| [`%chain`](https://puck.uno/requirements/chain/) | Anywhere | — | The ambient call-frame chain. Grants and revokes ride here; every chain-mediated capability is `%chain.X`. |
...
| [`%fetch`](https://puck.uno/requirements/chain/methods/puck) | Anywhere | Yes | Object download by URL. `%(url)` is the further shorthand. Shortcut for `%chain.puck`. |
...
| [`%stderr`](https://puck.uno/requirements/chain/methods/stdout-and-stderr) | Anywhere | No | Diagnostic-output channel. Grant to non-user code with `%chain.grant :stderr`. Shortcut for `%chain.stderr`. |
| [`%stdin`](https://puck.uno/requirements/chain/methods/stdin) | Anywhere | No | Input channel. Grant to non-user code with `%chain.grant :stdin`. Shortcut for `%chain.stdin`. |
| [`%stdout`](https://puck.uno/requirements/chain/methods/stdout-and-stderr) | Anywhere | No | Primary output channel. Grant to non-user code with `%chain.grant :stdout`. Shortcut for `%chain.stdout`. |

## Shortcuts and shorthands

- **`@foo`** — shorthand for `%bucket['foo']`. Method body only.
- **`%(url)`** — shorthand for `%fetch(url)`.
...
- **Clock and randomness are downloadable objects**, not globals — reach them via `%('core:now')` and `%('core:random')` (`%(...)` is the shorthand for `%fetch(...)`). Each call returns a fresh object; capture and use.
