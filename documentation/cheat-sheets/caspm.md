...
- **If-atom collapse** — `if / elsif / else` produces `{if: {conditions: [{test, action}], else: [...]}}`. `else` field omitted when the source has no else clause. Matches [§ If](https://puck.uno/requirements/caspianj#if).
- **Ternary and if share one shape** — source `cond ? then : else` normalizes to the same `{if: {conditions, else}}` atom the if-statement produces. Matches spec.
