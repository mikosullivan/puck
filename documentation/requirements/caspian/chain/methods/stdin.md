...
Why both surfaces exist: the user-only `%engine.stdin` is always available to the program itself; `%stdin` is what the user passes down the call chain when a library legitimately needs to consume input. Granting `%stdin` is opt-in by the user — libraries don't get it ambiently.
