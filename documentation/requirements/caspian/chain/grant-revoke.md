...
`%role.grant($role, :caps...)` grants capabilities to roles, not frames. See the full spec in [roles § Granting capabilities to other roles](https://puck.uno/documentation/requirements/caspian/roles/#granting-capabilities-to-other-roles).
...
- **Role-targeted `%role.grant`** — `%role.grant($role, :caps...)` grants capabilities to the role rather than the frame; see the role spec for its full semantics.
...
- **Role-targeted regrant needed for cross-role delegation** — inside a parent frame, `%role.grant($b_role, :caps) do ... end` makes role B see the capability across the boundary.
...
- **Grant/revoke interact with `%role`-scoped grants** — a `%role.grant` targeting a role and a `%chain.X.grant` in the current frame produce compatible state; neither shadows the other unexpectedly.
