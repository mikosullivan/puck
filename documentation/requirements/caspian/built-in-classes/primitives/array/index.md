...
- **Ruby-style `<<` as an append operator** — `$arr << $item` as sugar for `$arr.push($item)`. The token conflicts with the heredoc opener ([heredocs](https://puck.uno/documentation/requirements/caspian/built-in-classes/primitives/string/heredocs)): the parser can disambiguate by position (`<<` in a value-producing position is a heredoc opener; `<<` after a value expression is the binary append operator), but that adds complexity to the parser and every downstream tool that reads Caspian source (highlighter, formatter, LSP). The alternative is dropping `<<` as append entirely and keeping `.push($item)` as the only spelling — what Python, JavaScript, Rust, and most other languages do. Not decided. If `<<` append survives, the exact disambiguation rules and their edge cases (paren-less function calls, chained appends, `<<` as the first token of an expression) need to be spec'd.
