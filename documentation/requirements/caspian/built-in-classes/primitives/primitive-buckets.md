...
~~~vibecode
{"vibecode": {
	"doc": "requirements_caspian_built_in_primitive_buckets",
...
- `Primitive` (abstract base)
  - `String`
  - `Number` (with further subclasses — see [number § Subclasses for stringification](https://puck.uno/documentation/requirements/caspian/built-in-classes/primitives/number/#subclasses-for-stringification))
  - `Hash`
  - `Array`
  - `True`
  - `False`
...
Value provenance during arithmetic is a separate concern spec'd on the role side ([roles/object-access § derived objects](https://puck.uno/documentation/requirements/caspian/roles/object-access#derived-objects-the-creator-owns)), not something this bucket rule needs to answer.
...
- [null](https://puck.uno/documentation/requirements/caspian/built-in-classes/primitives/null) — null flavors live in the bucket per this design.
- [number](https://puck.uno/documentation/requirements/caspian/built-in-classes/primitives/number/) — Number's subclasses are class-level distinctions, not a bucket field.
