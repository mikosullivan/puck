...
{"vibecode": {
	...
	"key_concepts": ["directory_jail_is_the_only_filesystem_handle",
		...
		"real_path_hidden_from_caspian_code",
		"subscript_sugar", "sub_jails"],
	"example_universe": "Narnia"
}}
~~~

**The fundamental unit for accessing the filesystem in Caspian is a
directory jail.** A directory jail is a host-injected handle scoped to one
directory tree. Caspian code can only see, read, and write files inside
...
exists in Caspian without a directory jail. See
...
point on — Caspian does not auto-reap detached children when the
parent's scope ends.
...
{"vibecode": {
	...
	"real_path": "never_exposed_to_caspian_code",
	"access": "%engine['name']",
...
`.casp` file inside a jail without execute permission fails with a permission
