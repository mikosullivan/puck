...
- **Not the plumbing concept.** [Plumbing](https://puck.uno/documentation/requirements/caspian/plumbing/) covers faucets (inbound) and sinks (outbound) — runtime edges of the Caspian process, not a syntactic operator.
- **Not the shell pipe.** Shell features aren't part of `.execute`; see [linux-support](https://puck.uno/documentation/requirements/caspian/linux-support/).
- **Not bitwise-OR.** Bitwise operations live on the [`.bitwise`](https://puck.uno/documentation/requirements/caspian/built-in-classes/primitives/number/bitwise) wrapper on numbers, precisely so `|` is free for pipe use here.
...
`|` binds **looser than every other operator** — arithmetic, comparison, logical, and so on all evaluate first, and the pipe stages structure what happens at the top level. So:

~~~caspian
$result = $a + $b | &fn
~~~

evaluates as `$result = ($a + $b) | &fn` — arithmetic runs first, then the sum flows into `&fn`. **`$result`** is `&fn($a + $b)`. Same choice as Elixir's `|>`, F#'s `|>`, R's `%>%`.

**Style: use explicit parens when a pipe stage contains other operators.** Even though the precedence rule is unambiguous, an unparenthesized mix reads poorly at a glance. Prefer:

~~~caspian
$result = ($a + $b) | &fn
~~~

Same meaning, but the parens make the pipe boundary explicit and don't rely on the reader knowing where `|` sits in the precedence table.
...
- [operators](https://puck.uno/documentation/requirements/caspian/syntax/operators) — other operator syntax.
- [built-in-classes/primitives/number/bitwise](https://puck.uno/documentation/requirements/caspian/built-in-classes/primitives/number/bitwise) — why bitwise-OR uses the wrapper, freeing `|` for pipe.
