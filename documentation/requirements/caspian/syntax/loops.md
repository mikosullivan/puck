...
No `do` keyword between the condition and the body — control-flow structures own their body directly. This doc has the full `while`/`until` spec; [if-unless](https://puck.uno/documentation/requirements/caspian/syntax/if-unless) covers the sibling conditional keywords.
...
**If you want to hand across a controller without granting exit capability**, wrap it in a [jail](https://puck.uno/documentation/requirements/caspian/built-in-classes/object/methods/#jail) that exposes only the state readers:

~~~caspian
&untrusted_helper $loop.object.jail(:index, :count)
~~~

The recipient can read `.index` and `.count` (and any other state-reader you list) but can't reach `.break`, `.return`, or `.next` — the jail forwards only the named methods to the underlying controller. This is the standard [object-capability](https://puck.uno/documentation/requirements/caspian/roles/object-access) pattern applied to controllers.
