...
**ES256 / RS256 signature verify** calls the operator-installed [`openssl`](../../linux-support/openssl) binary directly via `.execute` — subprocess invocation, no shell involved (see [linux-support § Not literally shelling out](../../linux-support/#executing-files)). Zero bundled bytes; treated as a Caspian prerequisite in the same posture as `luarocks` (third-party Lua libs) and `tar` (self-test tarball extraction). Widely available on every mainstream Linux distro and on macOS. Documented in [linux-support/openssl](../../linux-support/openssl).
...
- [linux-support/openssl](../../linux-support/openssl) — the wrapper class that invokes the operator-installed `openssl` binary directly for ES256 / RS256 signature verify.
