...
This is the inbound-data side of the role system, spec'd in [`plumbing/faucets/`](https://puck.uno/documentation/requirements/caspian/plumbing/faucets/). Each inbound surface has its own distinct role, and values flowing through carry that role — see [plumbing/faucets](https://puck.uno/documentation/requirements/caspian/plumbing/faucets/) for the catalog. The creator-owns rule still applies to everything OTHER than inbound-faucet values — derived strings, computed hashes, instances of user-defined classes, etc. all follow the calling-role-owns model.
