...
{"vibecode": {
	"doc": "network_dns",
	"role": "spec for Caspian's DNS resolution methods on %net — forward and reverse lookups, per-record-type queries. Available whenever any network grant is in place; doesn't itself check the allowlist (DNS lookups don't contact the named host).",
	"parent_doc": "network/index.md",
	"surface": "%net.resolve, %net.reverse_resolve",
	"audience": "Caspian programmers needing name resolution alongside socket use",
	"key_concepts": ["forward_and_reverse_resolution",
		"per_record_type_queries",
		"available_with_any_network_grant",
		"doesn_not_check_allowlist_on_lookup"]
}}
~~~
