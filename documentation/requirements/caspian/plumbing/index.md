...
~~~vibecode
{"vibecode": {
	"doc": "requirements_caspian_plumbing",
...
- **[Faucets](https://puck.uno/documentation/requirements/caspian/plumbing/faucets/)** bring values IN. Every inbound value (stdin bytes, an env var, a network response, a `%puck` object) comes through a faucet with its own role, and the value carries that role.
- **[Sinks](https://puck.uno/documentation/requirements/caspian/plumbing/sinks/)** push values OUT. Every outbound method (a stdout write, an HTTP request body, a filesystem write) is a method on a sink object, and holding the object is authority to call it.
...
- [Faucets](https://puck.uno/documentation/requirements/caspian/plumbing/faucets/) — inbound edge; role-tagging of pulled values; narrowed-faucets rule.
- [Sinks](https://puck.uno/documentation/requirements/caspian/plumbing/sinks/) — outbound edge; sinks are just objects; holding-is-access; engine as the only outbound gateway.
