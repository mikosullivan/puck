...
- **Roles registry.** Named roles the engine knows about. Always includes at least `user` (the role the user program starts in). Built-in capability-holding roles like `stdlib`, `stdout`, `stderr` are registered too — they're the roles that own engine-provided functionality (the string class, the puts BWC, the stdout-writing handler, etc.). Roles are how cross-role calls (which require permission negotiation) are tracked; the registry is the source of truth for which exist.
