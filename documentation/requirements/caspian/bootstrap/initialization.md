...
- **Roles registry.** Named roles the engine knows about. The `user` role is the one the first user-program frame runs under. Beyond that, see [roles](https://puck.uno/documentation/requirements/caspian/roles/) for the catalog and [plumbing/faucets](https://puck.uno/documentation/requirements/caspian/plumbing/faucets/) for the per-faucet role list. The registry is the source of truth for which roles exist.
