...
Most user code reaches this through the global shortcut `%chain.net.http` (or further-shortened helpers like `%fetch.download`); `%engine.http` is the underlying surface those globals are built on.
...
- **`%engine.http.get(url)` returns a response object** — a successful GET has `.status`, `.headers`, and `.body`.
...
- **`%engine.http.post(url, body: ...)` sends the body** — the server observes exactly the bytes passed.
- **`%engine.http.put(url, body: ...)` sends a PUT** — verb mapping matches the method name.
- **`%engine.http.delete(url)` sends a DELETE** — verb mapping matches the method name.
- **`%engine.http.head(url)` returns headers with no body** — `.body` is empty; `.headers` present.
- **`%engine.http.patch(url, body: ...)` sends a PATCH** — verb mapping matches.
- **Custom request headers are sent** — `%engine.http.get(url, headers: {authorization: 'Bearer xyz'})` puts the header on the wire.
...
- **4xx status codes do NOT raise** — `%engine.http.get(url_returning_404).status` is `404`; no exception.
- **5xx status codes do NOT raise** — `%engine.http.get(url_returning_500).status` is `500`; no exception.
...
- **Unicode in URL paths is percent-encoded** — `%engine.http.get('https://example.com/path/Zoë')` reaches the correct URL server-side.
...
- **Non-user role calling `%engine.http` raises** — the `%engine` blanket gate.
- **Capturing `%engine.http` and calling from a non-user frame** — under method-runs-as-owner, the underlying call runs as the capturer's role; a non-user frame that directly calls `%engine.http` (never captured) raises.
- **Sending a request with a non-string body raises** — `%engine.http.post(url, body: 42)` raises rather than coerces silently.
- **A GET with a nil URL raises** — `%engine.http.get(null)` raises a validation error.
- **HTTP verb methods are read-only slots** — `%engine.http.get = ...` raises.
