...
Runs under the [self-test](https://puck.uno/documentation/requirements/installation/self-test/) framework — non-privileged, read-only, no side effects. Prints:
...
Discussed in detail at [ideas/caspian/sandboxing-primitives](https://puck.uno/documentation/ideas/caspian/sandboxing-primitives). Summary of what the engine does:
...
- **Not a full-syscall sandbox.** Sandbox restricts filesystem view only. A block inside a sandbox can still open network sockets, run subprocesses, allocate memory. Constraining those is the role of [grants](https://puck.uno/documentation/requirements/roles/) and `%chain`, which compose with sandboxing but are separate concerns.
...
- **What lives at `%fs['/tmp/foo'].sandbox`.** The `%fs['path']` handle already exists as the [dirs](https://puck.uno/documentation/requirements/filesystem/dirs/) surface. Adding `.sandbox` as a block-taking method on it is straightforward, but the interaction with existing `%fs` methods called on the same handle from outside the block needs to be pinned down. (Nothing should break; just needs the test cases.)
...
- [ideas/caspian/sandboxing-primitives](https://puck.uno/documentation/ideas/caspian/sandboxing-primitives) — mechanism proof-of-concept and prototype notes.
- [concepts § Lean on installed Linux utilities](https://puck.uno/documentation/requirements/concepts#lean-on-installed-linux-utilities-when-theyre-better) — the design principle this fits under (kernel is the "utility", AppArmor is the profile that lets Caspian reach it).
- [roles](https://puck.uno/documentation/requirements/roles/) — the grant model that composes with sandboxing for full-surface confinement.
- [filesystem/dirs](https://puck.uno/documentation/requirements/filesystem/dirs/) — the `%fs['/path']` handle that carries the `.sandbox` method.
- [linux/cli/openssl](https://puck.uno/documentation/requirements/linux/cli/openssl) — parallel case of a Caspian feature that leans on host-provided infrastructure.
