...
Per [concepts § Caspian is written in Caspian](https://puck.uno/documentation/requirements/concepts#caspian-is-written-in-caspian), this is exactly the case that principle carves out — *"reach for the host language only for what genuinely can't exist above the primitive line."* The engine's JSON parser is one of those.
...
- [core/](https://puck.uno/documentation/requirements/core/) — LPeg is bundled in the Executable tier; the engine JSON parser is one of its consumers.
- [concepts § Caspian is written in Caspian](https://puck.uno/documentation/requirements/concepts#caspian-is-written-in-caspian) — the design principle and its bootstrapping exception.
- [built-in-classes/primitives/hash](https://puck.uno/documentation/requirements/built-in-classes/primitives/hash/) — Caspian's ordered-hash type; the shape the parser constructs for JSON objects.
