...
- [cache-dir](https://puck.uno/documentation/requirements/cache-dir) — Cache-format directories, another fetcher class in the array. `meta.json`'s stored endorsement (when present) is what lets a Cache-served version be verified against the trust anchor.
- [content-types](https://puck.uno/documentation/requirements/content-types) — Content-Type strings the blockchain service uses in responses.
- [`%puck`](https://puck.uno/documentation/requirements/chain/methods/puck) — the gateway that walks `%puck.fetchers`.
