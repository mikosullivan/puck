...
- **[Bare function](bare)** — declared with `function`. No captured outer variables, no receiver. Two declaration forms with different source-level effect: `function &name(...) end` publishes the function as a method on the enclosing [Module](https://puck.uno/documentation/requirements/modules/) (ergonomic for mutually-referencing modules); `$var = function(...) end` binds a local variable and is fully hermetic (the security default).
...
Every step is an explicit choice by the author; there is no "default lexical capture" behavior. See [bare § Sibling access via %module](bare#sibling-access-via-module) and [modules](https://puck.uno/documentation/requirements/modules/) for the Module mechanism the named form plugs into.
