...
- **What's reachable.** The arguments passed in (via the parameter list), the names defined locally within the body, [`%chain`](https://puck.uno/documentation/requirements/chain/), and — when the function was declared with the named form (`function &name(...) end`) — the surrounding [Module](https://puck.uno/documentation/requirements/modules/)'s methods and classes via [`%module`](https://puck.uno/documentation/requirements/global-methods/module/).
...
3. **Its own Module via `%module`.** Because the body is inside SOME code-container, `%module` returns the innermost containing Module. What's on that Module — `%module.methods`, `%module.classes` — is only the source's explicitly-named declarations at that container's scope. See [modules](https://puck.uno/documentation/requirements/modules/) for what a Module exposes and (importantly) what it does NOT expose.
...
See [`%chain`](https://puck.uno/documentation/requirements/chain/) for how the capability side works — grant propagation, role boundaries, default-deny vs. default-grant per surface.
...
2. `name` is published as a method on the current [Module](https://puck.uno/documentation/requirements/modules/).
...
When two functions live in the same [Module](https://puck.uno/documentation/requirements/modules/) and need to reach each other, always go through `%module`:
...
A function object exposes a small set of methods directly, plus everything on the [`object` namespace](https://puck.uno/documentation/requirements/built-in-classes/object) that every value carries.
...
Full spec, including the block-invocation surface (`%call.blocks[N].call`) and the caller-object mechanism used for DSL-style blocks, is on [`%call`](https://puck.uno/documentation/requirements/global-methods/call/) and [caller](tag:caller).
