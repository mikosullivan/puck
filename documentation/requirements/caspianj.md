...
- **Bareword-command atoms** — `field`, `private`, `autorun`, etc. stay as `{bwc: "field"}` atoms rather than being resolved.
...
**Why the opt exists.** Line info exists to make runtime errors point at source. For the core-binary stdlib, that trade doesn't pay:

- The stdlib is written and shipped by Caspian; if a runtime error surfaces inside it, the fix lives in the transpiler / stdlib source, not in end-user code.
- End users can't see or edit the shipped CaspM anyway — a "line 42" message from inside a stdlib method is noise to them.
- Every byte in the core binary counts against the floppy budget.

For user code (application source, downloaded classes fetched at runtime), keep `{lines: true}` so runtime errors report the actual source line.

**Size impact estimate.** Line info dominates when it's present: every value atom carries `"l": N` (≈8 bytes with the short key) plus the multi-line-statement trailing metas. On a stdlib file that's ~30-40% of the bytes; stripping is a bigger single lever than key-shortening was.

**Why not always strip at the CaspJ level?** Because the same CaspJ may feed multiple downstream consumers — a cache holds one canonical CaspJ, and different builds (test runner needs lines, core-binary embed doesn't) normalize it differently. Stripping at normalize time keeps the CaspJ shape stable and puts the decision at the last step where it matters.

## Calls

<span class="tag">function-call-bwc</span>

Every call in Caspian — amp call (`&x`), method call (via dot), closure invocation, downloaded-method application — collapses in CaspM to a single internal primitive: `function_call` (`{in: "fc"}`). One shape, one dispatch. Amp calls (`&x`) become function_call with `receiver: x, function: "call"` — always dispatching to the receiver's `.call` method, no runtime property lookup. Dot method calls (`$foo.bar(...)`) become function_call with `receiver: {var:"foo"}, function: "bar"`. This unification matches the callables model where a function IS a method (see [callables idea](https://puck.uno/documentation/ideas/callables#function-is-method)) — the object is the same, invocation context varies.
