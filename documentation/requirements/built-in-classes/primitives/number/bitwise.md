...
~~~vibecode
{"vibecode": {
	"doc": "requirements_built_in_number_bitwise",
...
Every method returns a plain [number](https://puck.uno/documentation/requirements/built-in-classes/number/). Where a method takes a right-hand operand, it's a plain number.
...
- [Number](https://puck.uno/documentation/requirements/built-in-classes/primitives/number/) — the parent class; `.bitwise` is a method on every whole-value number.
- The [number § Predicates](https://puck.uno/documentation/requirements/built-in-classes/primitives/number/#predicates) — `.integer?`, `.even?`, `.odd?` — often paired with bitwise operations.
