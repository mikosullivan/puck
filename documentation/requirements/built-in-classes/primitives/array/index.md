...
The symmetric-indexing pattern relies on **[negative zero](https://puck.uno/documentation/requirements/built-in-classes/number/#negative-zero)**. `-0` is a distinct signed-zero value that equals `0` in every other context (arithmetic, comparison, hash keys) but reads as "0 from the end" when used as an array index.
...
| Method | Description |
|---|---|
| `.length` | Number of elements as a [number](https://puck.uno/documentation/requirements/built-in-classes/number/). |
...
`$arr <+ $item` appends `$item` to the array — equivalent to `$arr.push($item)`. The operator lives in [syntax/operators § `<+` append](https://puck.uno/documentation/requirements/syntax/operators/#-append); on an Array receiver it dispatches to the same code path as `.push`.
...
- **`.sample` draws from `%random`** — swapping `%random` changes the draw deterministically.
...
- **`.shuffle` draws from `%random`** — a fixed seed produces a deterministic permutation.
...
- [Syntax § Variables and assignment](https://puck.uno/documentation/requirements/syntax/variables-and-assignment) — assignment to array indices.
