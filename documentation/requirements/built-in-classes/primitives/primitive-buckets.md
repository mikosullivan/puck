...
- `Primitive` (abstract base)
  - `String`
  - `Number` (with further subclasses — see [number § Subclasses for stringification](https://puck.uno/documentation/requirements/built-in-classes/primitives/number/#subclasses-for-stringification))
  - `Hash`
  - `Array`
  - `True`
  - `False`
...
- **String content-type** — `@content_type = 'text/markdown'`. Lives in the bucket of a String instance. Exposed via String's `.content_type` / `.content_type=` methods; set by [heredocs](https://puck.uno/documentation/requirements/built-in-classes/primitives/string/heredocs#type-annotation) when the opener includes a `(type)` slot, or explicitly via the setter.
...
Value provenance during arithmetic is a separate concern spec'd on the role side ([roles/object-access § derived objects](https://puck.uno/documentation/requirements/roles/object-access#derived-objects-the-creator-owns)), not something this bucket rule needs to answer.
...
- [downloaded-methods](https://puck.uno/documentation/requirements/classes/downloaded-methods) — the spec that surfaced primitive-bucket access via `$n.$method`. This design cleanly supports that surface.
- [null](https://puck.uno/documentation/requirements/built-in-classes/primitives/null) — null flavors live in the bucket per this design.
- [number](https://puck.uno/documentation/requirements/built-in-classes/primitives/number/) — Number's subclasses are class-level distinctions, not a bucket field.
