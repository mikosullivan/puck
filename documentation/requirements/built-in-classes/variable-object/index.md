...
The `++` and `--` operators are the ONLY Caspian-source way to bump a variable. They desugar in CaspM to direct dispatches on four **CaspM-only** bwcs (`suffix_increment`, `prefix_increment`, `suffix_decrement`, `prefix_decrement`) — those bwcs are not directly callable from Caspian source. See [caspianj § Bumps](https://puck.uno/documentation/requirements/caspianj#bumps) for the full desugaring.
...
`$$name` produces the atom `{varobj: NAME}` in full CaspJ, parallel to `{var: NAME}` for the value form. See [caspianj § Full](https://puck.uno/documentation/requirements/caspianj#full).
...
_No open items — closure-capture behavior settled in [Interaction with closures](#interaction-with-closures); block-scope walk-then-write behavior spec'd via the scope runtime (see [lua/scope § Assignment walks the scope agg](https://puck.uno/documentation/requirements/lua/scope#assignment-walks-the-scope-agg))._
...
- [syntax/variables-and-assignment](https://puck.uno/documentation/requirements/syntax/variables-and-assignment) — variable declaration, value assignment, scoping rules.
- [syntax/sigils](https://puck.uno/documentation/requirements/syntax/sigils) — the sigil family (`$`, `$$`, `%`, `@`, `&`).
- [caspianj § Calls](https://puck.uno/documentation/requirements/caspianj#calls) — how variable-objects fit into the function_call primitive, especially for bump operators.
- [lua/scope](https://puck.uno/documentation/requirements/lua/scope) — the scope runtime that owns variable slots; interaction with `$$foo` still to be pinned.
