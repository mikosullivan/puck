...
The namespace is a [nested method namespace](https://puck.uno/documentation/requirements/classes/nested) inherited by every subclass automatically — user classes don't need to declare `nested :object` themselves. Nothing about the namespace's semantics differs from a nested namespace on a user class:
...
- [nested methods](https://puck.uno/documentation/requirements/classes/nested) — the general spec for nested method namespaces, which `object` follows.
