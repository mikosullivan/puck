...
Methods in the `object` namespace apply to any value in Caspian. They inherit the [nested method namespace semantics](https://puck.uno/documentation/requirements/classes/nested): each is a normal method on the receiver with full instance access, dispatched through the dotted path `$foo.object.method`.

## Method surface

| Method | Description |
|---|---|
| `.truthy?` | Returns the receiver's truthiness, always as a boolean. Derived from the receiver's [primitive field](https://puck.uno/documentation/requirements/built-in-classes/object/structure/#primitive-field): `false` and `null` primitives read as falsy; everything else (including any other primitive value, or no primitive field at all) reads as truthy. |
...
The engine reads the receiver's [primitive field](https://puck.uno/documentation/requirements/built-in-classes/object/structure/#primitive-field) and applies the rule:
...
The value follows the general [truthy/falsy rule](https://puck.uno/documentation/requirements/syntax/truthy-and-falsy): only `null` and `false` are falsy; everything else is truthy. `.truthy?` returns the same value the language uses when the receiver is evaluated as a condition.
...
**Downloaded methods excluded.** Ad-hoc method application via `$foo.$fn` ([downloaded-methods](https://puck.uno/documentation/requirements/classes/downloaded-methods)) is a call-site application, not a class-level attachment. Such applications do not appear in `.methods` — the class is the source of truth, and downloaded methods never modify the class.
...
**Nested namespaces surface as single entries.** A nested method namespace (defined via [nested](https://puck.uno/documentation/requirements/classes/nested), including the cross-cutting `object` namespace every value carries) appears as one entry keyed by the namespace name. Its members do not get flattened into the top-level methods list. To enumerate a nested namespace's methods, access the namespace and query its own `.methods`:
...
- [syntax § truthy and falsy](https://puck.uno/documentation/requirements/syntax/truthy-and-falsy) — the language-level rule that `.truthy?` reports.
- [roles § object-access § Narrowing](https://puck.uno/documentation/requirements/roles/object-access#narrowing-pass-a-jail-not-the-raw-object) — how jail wrappers are used across role boundaries.
