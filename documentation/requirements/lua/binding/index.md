...
Caspian's runtime is Lua under the hood, but Caspian is its own language and Caspian code doesn't speak Lua directly. When a developer wants to use a Lua library — luaexpat, cjson, luasocket, or anything they've installed via `caspian --install-lua` — the language needs a way to reach into the Lua world and call that library's functions. This page specs how.
...
Under the hood, `%lua['lxp']` calls Lua's `require('lxp')` and wraps the returned value as a Caspian object. (Luaexpat's Lua-side module name is `lxp`, not `luaexpat` — the package installs as `lua-expat` on Debian / `luaexpat` on luarocks, but `require`'s the module as `lxp`.)

`$xml` is a Caspian wrapper object whose properties and methods correspond to the Lua module's exports. If luaexpat's Lua-side surface exposes a `new` factory returning a parser with `parse` and `close` methods, then from Caspian:

~~~caspian
$parser = $xml.new $callbacks
$parser.parse $chunk
$parser.close
~~~
...
Because the surface area is that small, a single wrapper class can handle every Lua library. It doesn't need to know about luaexpat or cjson or luasocket specifically — it just needs to know how to look up a field in a table, call a function, index by an arbitrary key, and delegate through metatables when present.

### What the wrapper actually does

Say `$xml = %lua['lxp']`. Then:

- **`$xml.new`** — wrapper's property-access looks up `"new"` on the wrapped Lua table, gets back a Lua function, wraps that function in another wrapper instance (Lua functions are values too), returns the wrapper.
- **`$xml.new $callbacks`** — wrapper's method-call: marshals `$callbacks` (Caspian hash → Lua table), invokes the underlying Lua function, wraps the return value on its way back.
- **`$xml[$key]`** — wrapper's index: marshals `$key`, does `lua_table[key]`, wraps the result.

For chained access like `$parser.callbacks.StartElement`, each `.callbacks`, `.StartElement` is a property lookup on the previous wrapper. Each returns a new wrapper for the intermediate table. The chaining works because wrappers-around-tables are indistinguishable from wrappers-around-any-other-table.
...
- **Lua's own `require` caches** the module — repeated `require('lxp')` calls all return the same underlying Lua table.
- **The Caspian wrapper is not cached** — each `%lua['lxp']` call produces a fresh wrapper instance around the (same) underlying table.
...
$callbacks = {
	StartElement: closure ($parser, $name)
		if $name == 'title'
			$in_title = true
		end
	...
	EndElement: closure ($parser, $name)
		if $name == 'title'
			$in_title = false
		end
	...
	CharacterData: closure ($parser, $text)
		if $in_title
			$titles.push $text
		end
...
$parser = $xml.new $callbacks
$parser.parse '<rss><channel><item><title>Hello</title></item></channel></rss>'
$parser.close
