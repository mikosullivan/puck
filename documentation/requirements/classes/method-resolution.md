...
The platter stack, and the "top is index 0, dispatch walks top-to-bottom" ordering, are spec'd on [object structure § Stack](https://puck.uno/documentation/requirements/built-in-classes/object/structure/#stack). This page adds the rule for what happens inside each platter.
...
- **Shadow platter.** Spec'd on [object structure § shadow](https://puck.uno/documentation/requirements/built-in-classes/object/structure/#shadow). The shadow sits at position 0 by convention and participates in dispatch like any other class-carrying platter — no MI-specific rule.
- **`%self`.** Binds to the instance receiving the call, regardless of which class defines the method that runs. Not affected by MI.
- **Field storage.** This page specs method dispatch only. Bucket and field storage are spec'd on [object structure](https://puck.uno/documentation/requirements/built-in-classes/object/structure/); field-name ownership across MI is a separate concern.
