...
They're not interchangeable; `%call.role` is specifically the third one. See [roles/object-access § Class instantiation is not an exception](https://puck.uno/documentation/requirements/roles/object-access#class-instantiation-is-not-an-exception) for the broader breakdown.
...
The method body runs in the class's role; `%call.role` is the caller's role; the comparison decides whether to proceed. The full rule for cross-role method access lives in [roles/object-access](https://puck.uno/documentation/requirements/roles/object-access#self-gating-from-inside-the-method).
...
See [classes/definition § Private methods](https://puck.uno/documentation/requirements/classes/definition/#private-methods) for the full private-method spec and [functions/method § Calling sibling methods](https://puck.uno/documentation/requirements/functions/method#calling-sibling-methods) for the sibling-call surface.
