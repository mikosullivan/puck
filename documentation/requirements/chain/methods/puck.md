...
Caspian doesn't have a "library" concept as a technical primitive — `%puck` downloads objects (classes, instances, records, etc.). See [concepts § Objects, not libraries](https://puck.uno/documentation/requirements/concepts#objects-not-libraries) for the framing.
...
- **Bracket shorthand returns object** — `%puck['https://example.com/widget']` returns the object at that URL when the URL is reachable and serves a Caspian-parseable body.
- **`%[url]` further shorthand** — `%['https://example.com/widget']` is equivalent to `%puck['https://example.com/widget']` and returns the same object.
- **`%puck.lookup` long form** — `%puck.lookup('https://example.com/widget')` returns the same object as the bracket shorthand.
- **Two calls, two independent objects** — `$a = %puck['url']; $b = %puck['url']` produces distinct objects; mutating a property on `$a` does not affect `$b`.
...
- **Method call triggers dependency download** — a downloaded object whose method body calls `%puck['other-url']` fetches the dependency only when that method is invoked, not at the original download.
- **Byte cache hit skips network** — after a first successful download, a second `%puck['url']` in the same run does not issue a second network fetch; the bytes come from the cache.
...
- **Default owner is the `%puck` faucet's role** — inside a downloaded object's method, `%chain.role` (or equivalent introspection) reports the puck-faucet role, not the caller's role.
...
- **`as_self` does not transit** — an `as_self: true` object whose method calls `%puck['other-url']` without `as_self:` gets a further object owned by the puck-faucet role, not the caller's.
- **`%puck.register` makes URL resolvable** — after `%puck.register('https://example.com/widget', obj)`, `%puck['https://example.com/widget']` returns objects backed by that registration.
- **`%puck.fetch` returns raw bytes** — `%puck.fetch('https://example.com/widget')` returns the on-the-wire byte representation, not a usable object; the object layer is bypassed.
- **404 raises** — `%puck['https://example.com/does-not-exist']` against a URL that returns HTTP 404 raises a puck-lookup error.
...
- **Unicode URL path** — `%puck['https://example.com/wídget']` (non-ASCII in the path) resolves normally with proper percent-encoding.
...
- **Missing URL arg raises** — `%puck[]` and `%puck.lookup()` both raise for missing required argument.
- **Wrong-type URL arg raises** — `%puck[42]` (integer where URL string expected) raises a type error.
- **Empty-string URL raises** — `%puck['']` raises a puck-lookup error (empty URL is not a legal identifier).
...
- **Default-granted across role boundaries** — a non-user role invoked without an explicit `%puck` grant can still call `%puck['url']` (this capability is default-granted).
- **Fresh object each call under registration** — after `%puck.register('url', factory)`, repeated `%puck['url']` calls each produce a fresh object from the factory.
