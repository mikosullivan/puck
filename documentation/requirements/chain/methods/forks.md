...
The full forking spec — manager API (`.kill`, `.term_kill`, `.detach`, `.active?`, `.zombie?`, etc.), IPC, signal handling — has its canonical home under `requirements/forking/`. This page is just the entry-point surface.
