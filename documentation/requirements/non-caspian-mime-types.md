...
- [content-types](https://puck.uno/documentation/requirements/content-types) — canonical Content-Type strings for Caspian source and CaspianJ tree files, plus how servers should return them.
- [cache-dir](https://puck.uno/documentation/requirements/cache-dir) — how the cache stores non-Caspian content alongside its Content-Type in `meta.json`. The empty-content rules on this page apply when the cache serves a stored version back to a caller.
- [`%puck`](https://puck.uno/documentation/requirements/chain/methods/puck) — the Caspian-side gateway for URL fetches. `%puck.parsers` sits alongside `%puck.maps` and other `%puck.X` state.
