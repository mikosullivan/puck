...
`@field` names an entry in the current object's bucket — shorthand for `%bucket['field']`. Only meaningful inside a method body, where `%bucket` exists (see [method](https://puck.uno/documentation/requirements/functions/method)); using `@field` in a bare function or closure body raises.
...
`%name` names a system method — a surface the engine provides. `%self`, `%chain`, `%engine`, `%fetch`, `%call`, `%bucket`, `%stdout`, `%now`, `%net`, and so on. The full list lives under [global-methods](https://puck.uno/documentation/requirements/global-methods/).
...
Most `%X` names are sugar for `%chain.X` — `%now` is `%chain.now`, `%net` is `%chain.net`, and so on. See [%chain](https://puck.uno/documentation/requirements/chain/) for the full mechanism, including capability propagation and role boundaries. A small number of `%X` names live outside `%chain` (`%call`, `%engine`, `%self`, `%bucket`); those pages spell out where each one hangs.
