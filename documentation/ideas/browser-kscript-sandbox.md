# KScript in the browser

```
vibecode: {
    "status": "active_brainstorm",
    "started": "2026-05-17",
    "subsystem": "browser_kscript_engine",
    "purpose": "run_an_entire_kscript_processor_inside_a_web_browser",
...
vibecode: {
    ...
    "first_contact_alignment": "matches_feedback_first_contact_strategy_user_experiences_kscript_before_installing"
}
```

A KScript engine that runs entirely in the browser unlocks:
...
- **Browser REPL** — a single page that lets someone type KScript at a
...
let people experience KScript before buying into the whole system.
...
vibecode: {
    ...
    "for_community_and_portability": "ts_engine_consuming_kscriptjson_only",
...
### Artifact 2: TS KSJ-only engine — community and portability

A TypeScript engine that **consumes canonical KScriptJSON** and
executes it. No KScript source parser. No transpiler. Just the
runtime: bootstrap, materialize, dispatch, role transition, built-in
classes.

- **Pros**: small scope (parallels V0.01 Lua engine exactly —
  KSJ-only execution); community-readable mainstream-language
  reference implementation; validates the canonical KSJ spec by
  forcing it to be precise enough for two implementations to agree;
  drop-in for web tooling that has KSJ in hand and wants to execute
  it.
- **Cons**: a parallel implementation to keep in semantic sync with
  the Lua reference engine; doesn't accept KScript source on its
  own.
- **Use case**: a JS/TS project that has a KSJ tree (hand-written,
  ...
  for "how does a KScript engine work" without the parser
  complexity.
...
A TS-only KScript parser, planned in
...
A **hybrid** (TS parser handing KSJ to a WASM-Lua engine) was on the
...
- **The role model is a perfect sandbox fit.** KScript already has a
  role-based capability system designed for "run untrusted code with
  a restricted surface" (see [roles.md](../kscript/roles.md)).
  Browser-embedded KScript is exactly that use case. Just don't grant
...
- **Bundle size budget.** What's an acceptable payload for a
  ...
  (rules out tight pages); ruby.wasm is ~10MB; a hand-tuned KScript
