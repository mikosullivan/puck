...
The coarse trust model (one tag per value, see [trust.md](../trust.md)) collapses
construction history down to a single bit: tainted or not. That's enough for
...
- **Scope.** Does provenance apply only to strings, or also to numbers, hashes,
  ...
  source. Containers have their own per-element concerns we already flagged in
  trust.md.
- **Opt-in mode.** A reasonable design: provenance tracking enabled only when the
  ...
  the richness opt in. The KScript program doesn't notice the difference because
