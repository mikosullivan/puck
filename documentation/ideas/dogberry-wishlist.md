...
- The KScript runtime (one long-lived interpreter for the installation's
  lifetime)
...
1. **The basic installation stays very simple.** A directory of `.kscript`
...
Invocation uses the KScript runtime's general
[file-invocation model](../kscript/modules.md#invoking-a-file): a file is
...
3. **Reconcile request shape with what's at the path**:
   - File request + entry is a file → serve it (invoke if `.kscript`,
...
- **What "serving a directory" means.** When a directory request hits
  ...
  index file (`index.kscript`, `index.html`, etc. by priority),
...
content, internal KScript files that Robinson itself invokes.
...
invoked as a KScript file in some internal context). What's prohibited
...
- **Blocked from request**: `robinson.json`, `robinson.lock`,
  `robinson.kscript`, `robinson.html`, etc.
...
`index.kscript`), it disappears permanently for that site. Missing URLs
