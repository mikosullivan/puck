...
Per the [trust model](../trust.md), a database can be marked as a trusted
source. When an object is fetched from a trusted database, its source tag
is "the database" and its trust level is whatever the engine maps that
source to — potentially fully trusted.
