...
**Status:** brainstorm. A pure-Caspian PNG handler available as a Puck
...
- **No native dependency.** No FFI, no shelling out to `libpng` or
  ImageMagick. Works wherever Caspian runs.
- **Foundation for image work.** PNG is the on-ramp to broader image
  handling in Caspian; other image classes can layer on the same primitive.
- **Reference exists.** [chunky_png](https://github.com/wvanbergen/chunky_png)
  ...
  Ruby — Caspian should land in a comparable range.
...
2. **PNG bytes ↔ Caspian image object.** Encode and decode the PNG container
...
- **Deflate.** PNG IDAT chunks are zlib-compressed. Pure-Caspian deflate is
...
- **CRC32.** PNG uses CRC32 per chunk — another small primitive to implement
  in pure Caspian. Useful beyond PNG too.
