...
The stated principle is [concepts § Caspian is written in Caspian](https://puck.uno/documentation/requirements/concepts#caspian-is-written-in-caspian) — "anything that could be written in Caspian without giving up security, correctness, or usable performance should be." XML is exactly the shape that principle points at: a well-understood grammar, no cryptographic primitive to sink to C, no OS-service dependency. If Password can be Caspian, so can this.
...
- **Alternate encodings.** UTF-8 in, UTF-8 out (matches Caspian's [strings-are-UTF-8](https://puck.uno/documentation/requirements/concepts#strings-are-utf-8) posture). The `encoding=` attribute on the XML declaration is read and reported; a non-UTF-8 declaration raises rather than silently transcoding.
...
- `$element.each() as $child` — Caspian's [array iteration](https://puck.uno/documentation/requirements/syntax/loops) surface
...
- [json](https://puck.uno/documentation/requirements/json) — the parallel case that IS committed. Bootstrap constraint forces Lua-side; user-facing class is Caspian. Sets the pattern this doc mirrors.
- [core/](https://puck.uno/documentation/requirements/core/) — tier definitions and the floppy budget any Executable-tier proposal has to fit inside.
- [lua/binding/](https://puck.uno/documentation/requirements/lua/binding/) — the committed reach-path via `%lua['lxp']` (luaexpat, binding to Linux's installed `libexpat`).
- [concepts § Caspian is written in Caspian](https://puck.uno/documentation/requirements/concepts#caspian-is-written-in-caspian) — the design principle this doc is applying to XML.
- [stdlib-suggestions-review](https://puck.uno/documentation/ideas/caspian/stdlib-suggestions-review) — the V1 stdlib scoping table where `caspian.uno/xml.casp` currently lives as Ships-yes.
- [downloads/](https://puck.uno/documentation/requirements/downloads/) — the shape most V1 first-party downloads take; models the Cache-tier packaging option.
