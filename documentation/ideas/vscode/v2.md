# Caspian VS Code Extension — V2 ideas
...
formatting — six small rules, masked-string pre-pass, no Caspian parser
in TS. See [vscode-extension.md](vscode-extension.md) for that spec.

V2 is for ambitions that need either (a) a Caspian parser in TypeScript
...
{"vibecode": {
    ...
    "v2_options": ["typescript_caspian_parser_embedded",
                    "language_server_protocol_with_caspian_server"],
    "tradeoff": "lsp_unlocks_more_features_but_requires_a_running_caspian_capable_server_which_re_introduces_the_puck_dependency_unless_the_server_is_also_in_typescript"
}}
~~~
...
<a id="option-a-typescript-based-caspian-parser-still-no-external-runtime"></a>
### Option A: TypeScript-based Caspian parser, still no external runtime

A real Caspian parser written in TypeScript, embedded in the extension.
...
Cost: a second Caspian parser to maintain (the Lua reference engine has
the canonical parser; this would be a parallel TS implementation). The
two implementations must agree about what's valid Caspian.
...
- **Spec validation.** Two independent implementations agreeing forces
  the [CaspianJ](../caspian/caspianj.md) spec to be precise
...
- **Cross-tool config sharing.** V1 reads from VS Code's
  `settings.json` only. Other formatting consumers (Gitter, Differ)
  read `~/.config/caspian/style.json`. V2 might let the extension
  optionally read the external file as a fallback so a viewer who
  already maintains one file for Gitter/Differ doesn't have to
  re-enter the same preferences in VS Code settings.
- **Where the parser lives in the repo.** If V2 introduces a TS-based
  Caspian parser, does it live in `vscode/syntax/src/`? Or a peer
...
- **Reconciliation with the canonical Lua parser.** Two parsers for
  ...
  fixtures (the same `.casp` files must parse identically on both
...
- **Whether `vscode-extension.md` (V1) and this file should
  ...
  `documentation/caspian/vscode/` (replacing the current zero-byte
  `vscode/syntax/index.md`), or stay separate as "shipped" vs
