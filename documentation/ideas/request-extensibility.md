...
[`core:auth/api`](https://puck.uno/documentation/requirements/protected/auth/api) handles the "credential template in a header" pattern — Bearer tokens, API-key-in-header, simple HMAC over a fixed string. Other schemes need more:
...
Caspian's [class inheritance](https://puck.uno/documentation/requirements/classes/inheritance) supports either. Which pattern the ecosystem gravitates toward depends partly on what extension points the core exposes — if custom helpers require subclassing, that pushes toward subclass; if they don't, compose is often cleaner.
...
Inherits from `header.casp` and [`core:array`](https://puck.uno/documentation/requirements/built-in-classes/primitives/array/) via multiple inheritance. For headers whose value is a list of items joined by a separator — Accept, Accept-Language, Accept-Encoding, Cache-Control, etc.
...
Inherits from `header.casp` and [`core:hash`](https://puck.uno/documentation/requirements/built-in-classes/primitives/hash/) via multiple inheritance. For headers whose value is a set of name=value pairs joined by a separator — Cookie is the canonical example.
...
Spec'd on the [request page](https://puck.uno/documentation/requirements/http/request).
...
- **Authorization** — via [`core:auth/api`](https://puck.uno/documentation/requirements/protected/auth/api), not a plain request helper.
...
- [`core:http/request`](https://puck.uno/documentation/requirements/http/request) — the base class this page brainstorms extensions for.
- [`core:auth/api`](https://puck.uno/documentation/requirements/protected/auth/api) — the first "extension" of the request pattern (auth-scoped `.auth_headers` slot and send-time credential composition).
- [class inheritance](https://puck.uno/documentation/requirements/classes/inheritance) — the mechanism SDKs would use to subclass the request if that's the pattern.
