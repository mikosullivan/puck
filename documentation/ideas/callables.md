...
The closure's captured scope stays alive as long as the closure does — including whatever resources happen to be bound in that scope. The observable behavior is spec'd at [functions/closure § Captured scope keeps resources alive](https://puck.uno/documentation/requirements/functions/closure#captured-scope-keeps-resources-alive); the design commentary about why the trade-off looks the way it does lives here.
