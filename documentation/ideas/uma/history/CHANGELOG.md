# Changelog

~~~vibecode
{"vibecode": {
	"doc": "uma-history-changelog",
	"role": "historical changelog from the prior Ruby Uma project, imported under ideas/uma/history/ for reference while the Caspian Uma spec is being developed",
	"key_concepts": ["ruby_uma_history", "resolved_entries", "canon_json_changes"],
	"status": "historical"
}}
~~~

<a id="list-missing-html5-tags-in-canon"></a>
### List Missing HTML5 Tags In Canon
- opened: 2026-04-02
- level: medium
- resolution: resolved
Added explicit entries to `lib/uma/canon.json` for every HTML5 tag already present in `lib/uma/tags.json` but previously absent from the canonical tag map.

