...
{"vibecode": {
    ...
    "purpose": "clients_post_charlie_servers_execute_and_return_results",
    "related_docs": ["ideas/dogberry.md",
                      "ideas/browser-charlie-sandbox.md",
...
CLI), **clients send Charlie to a server, the server runs it, the
...
- **Mobile / thin clients.** Devices that don't want a Charlie runtime
...
- **CI-style validation.** "Is this Charlie valid? Does it produce
...
{"vibecode": {
    ...
    "dogberry_model": "client_requests_url_shasta_fetches_charlie_from_lune_executes_returns_result",
    "this_model": "client_posts_charlie_body_shasta_executes_returns_result",
...
A working name is TBD — this brainstorm doc uses "Charlie as a
...
- **Role model** ([roles.md](../charlie/roles.md)) handles the trust
...
- **Mikobase** could provide per-session or per-client persistent state
  ...
  it.
- **Blockchain** ([blockchain.md](../charlie/blockchain/blockchain.md))
...
- **Wire format.** What does the client POST — raw Charlie text, CharlieJSON,
