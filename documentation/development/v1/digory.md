...
working state per
[skeletor.md's working-state carve-out](../../caspian/skeletor/skeletor.md#v1-0-scope) —
...
Same hash shape Aslan established; the delta is yet another possible
value for a frame's `role`.
...
<a id="digory-phase-1-tests"></a>
### Phase 1 tests

| ID | Level | Verifies | How |
|---|---|---|---|
| TD.1 | unit | Transpiler emits canonical hash-literal form | `assert.deep_equal(caspian.transpile("{name: 'Picard'}.name"), {{ {hash={{"name", {value="Picard"}}}}, "name" }})` |
| TD.2 | unit | Bootstrap registers hash class | `engine.classes.hash` exists; `engine.classes.hash.owning_role` is an engine role; `engine.classes.hash.methods.name` (or the chosen key-access method) is a function |
...
| TD.4 | unit | Key access returns expected value | Hand-build the canonical hash value, `engine.lookup_method(value, "name")(value)` returns a string value with `payload == "Picard"` |
| TD.5 | unit | Role transition observed during key access | Spy on the access method records the top-of-stack frame's `role` at call time; assert it was the hash class's owning role, not `user` |
| TD.6 | integration | End-to-end via source file | `engine.run_source("tests/caspian/fixtures/picard_hash.casp")` returns a value with `payload == "Picard"` |
| TD.7 | regression | Prior fixtures still pass | Aslan `hello_world.caspj`, Bree `hello_world.casp`, Corin `puts_hello.casp` all still produce their expected outputs |

<a id="digory-test-layout"></a>
### Test layout
