...
{"vibecode": {
	"section": "outline",
	"role": "nested <ul>/<li> tree with custom bullets, hanging-indent for long entries, and an optional CSS-only expand/collapse toggle per branch; primary use case is page TOCs but the pattern is generic",
	"js_required": false,
	"primary_use_case": "TOC trees"
}}
~~~

A nested-list tree where each entry pairs a small "bullet or toggle" glyph with a link, branches indent under their parent, wrapped link text hangs under the first character of the text (not under the bullet), and any branch with children can expand/collapse via a CSS-only checkbox toggle — no `<details>` (so the toggle and the link can be separate click targets at every level).
...
<ul class="outline">
	<li>
		<input type="checkbox" id="t-1" class="show-nested">
		<label class="outline" for="t-1"></label>
		...
		<ul>
			...
			<li>
				<label class="outline">•</label>
				<a href="/overview/design">Design principles</a>
			</li>
		</ul>
	</li>
	<li>
...
- **Leaf** — `<label class="outline">` carrying the bullet glyph (e.g. `•`) followed by the link.
- **Branch** — hidden `<input type="checkbox" class="show-nested">` + `<label class="outline" for="...">` (the toggle target, glyph supplied by CSS `::before`) + the link + a nested `<ul>` containing children.

The `<input>` and `<label>` are siblings; the `<input>` is hidden via CSS. Clicking the label toggles the checkbox via the `for=` association, which flips a `:checked` selector that hides or shows the nested `<ul>`.
...
ul.outline ul {padding-left: 1.4em}
ul.outline li {
	padding: 1px 0 1px 1.5em;
	text-indent: -1.5em;
}
ul.outline label.outline {
	display: inline-block;
	width: 1em;
	text-align: center;
}
ul.outline label.outline[for] {cursor: pointer}
ul.outline .show-nested {display: none}
ul.outline .show-nested:not(:checked) ~ ul {display: none}
ul.outline .show-nested + label.outline::before {content: "\2295"}  /* ⊕ collapsed */
ul.outline .show-nested:checked + label.outline::before {content: "\2296"}  /* ⊖ expanded */
~~~

The `padding-left: 1.5em` + `text-indent: -1.5em` pair gives a hanging indent: the bullet sits at the natural left edge, wrap lines indent past it so the text aligns under the first character of the first line — no `flex` or `grid`, just two well-known properties.

The toggle glyph comes from `content` in CSS `::before` on the `<label>`, so the markup carries no glyph for branch labels — the CSS decides whether to draw `⊕` (collapsed) or `⊖` (expanded) based on `:checked`.

#### JavaScript

Zero. Expand / collapse, hanging indent, hover state, all CSS. The checkbox state lives in the DOM, so deep-linked URLs that should land on a particular open path can also be expressed in HTML (server-side rendering can stamp `checked` on the right inputs).

#### Why not `<details>`?

`<details>` couples the toggle to the entire `<summary>`. In a TOC, the row contains both a navigation link and an expand affordance, and those should be independent click targets — clicking the link should go to the page without toggling; clicking the toggle should expand without navigating. Checkbox + label keeps the two roles separate.

#### Real-world use

Orlando's auto-TOC on every doc page uses this exact pattern; its sidebar navigation tree uses a closely-related variant (with `<details>` instead, because the sidebar links and toggles share a row by design).
