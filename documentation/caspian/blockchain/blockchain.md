...
- `grammar` — Description of grammar being used.
...
There is no cutoff for switching to a new grammar. Each block
carries its own grammar reference, so old and new grammars coexist
indefinitely — parsers handle every block against whichever grammar
that block points at. If a grammar needs updating, a new one gets
published; existing blocks keep working against the grammar they
were written for.
