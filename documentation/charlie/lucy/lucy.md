...
Charlie has been in development conceptually for approximately twenty years.
...
merely awkward to write in Charlie.

The exception is pragmatic: if something could be in Charlie but has become unwieldy or
too slow in practice, move it to Lua. Performance and maintainability are valid reasons
to drop down. But the default is always Charlie first.

A compliant Charlie engine must provide a minimal kernel:
...
The Charlie parser is a hand-written recursive descent parser written in Charlie JSON
...
Charlie is not minimal in the way Lua is minimal, but it borrows Lua's discipline around
size. The target for Charlie's own code is 500k, not counting external dependencies
(Lua, SQLite, libmicrohttpd). If the compiled size of Charlie's own code exceeds 500k,
that is a red flag worth investigating. Every engine that runs Charlie gets mikobase support,
...
mikobases, and the multi-process coordination model are opt-in Charlie features the
...
[`?` suffix convention](../charlie.md#the-suffix): falsey on the happy path,
...
- Make each default-mode `%utils.timeout` call undefeatable by Charlie code —
  there must be no path from a Charlie expression to suppress a `timeout_handle`
- Allow `unwind: true` timeouts to be caught and suppressed from Charlie code
...
- **Default mode:** the error is tagged as `timeout_handle`. Charlie's
...
Charlie code cannot defeat the default-mode timeout because `debug`, `load`,
`os`, and the rest of Lua's standard library are not reachable from Charlie
...
By convention, code is shared as Charlie source. CharlieJSON is the runtime artifact,
not the distribution format.

Alternative syntaxes that transpile to CharlieJSON are possible but not encouraged. Charlie
is the language; CharlieJSON is the wire format.
...
Encoding is handled at the engine boundary, not in Charlie code. A compliant Charlie
engine must convert all incoming strings to UTF-8 before passing them to Charlie.
...
termination, no Charlie-level code is trusted to run during cleanup).
...
[jasmine/charlie.md § Automatic exception recording](../packages/jasmine/charlie.md#automatic-exception-recording).
...
Charlie. The only other multi-clause structure is `if`/`elsif`/`else`/`end`,
...
Implementors building Charlie on a host language should build the
...
- **Pattern matching (`match value ... case ... case ... end`)** — if
  Charlie ever adds pattern matching, each `case` clause is a
...
- **`as $name` is currently described in [charlie.md § The `as`
  Keyword](../charlie.md#the-as-keyword) as its own
...
The trailing `?` is the Charlie convention for predicate methods (boolean-returning).
...
`:get` and `'get'` are equivalent — `:foo` is shorthand for `'foo'` throughout Charlie.
...
objects. In Charlie, all functions are already objects.
...
This section documents the underlying dispatcher mechanism. For Charlie's language-wide
