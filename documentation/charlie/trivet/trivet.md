...
- [Node IDs](#node-ids)
  - [Tree-wide uniqueness](#tree-wide-uniqueness)
  - [Cross-tree behavior](#cross-tree-behavior)
...
- [Relationships](#relationships)
  ...
  - [Sugar for rehome](#sugar-for-rehome)
  - [What rehome doesn't do](#what-rehome-doesnt-do)
...
- [Traversal](#traversal)
  ...
  - [Walking only node objects](#walking-only-node-objects)
- [Query](#query)
  - [Stopping descent at matches](#stopping-descent-at-matches)
  - [`query_first`](#query_first)
- [Mutation](#mutation)
- [Child validation](#child-validation)
- [Output](#output)
- [Document](#document)
  - [Document vs root](#document-vs-root)
  - [Why a Document is useful](#why-a-document-is-useful)
  - [Document API](#document-api)
  - [`$node.document` from inside the tree](#nodedocument-from-inside-the-tree)
- [Subclassing](#subclassing)
- [Bucket discipline: the `uns` slot](#bucket-discipline-the-uns-slot)
...
over directly; method naming follows Charlie conventions.
...
the slot itself is universal — `$node.id` is always a meaningful
property to read or write.

```
$node.id              # returns the current id, or null
$node.id = 'food'     # sets the id
```

<a id="tree-wide-uniqueness"></a>
### 5.1 Tree-wide uniqueness

**Within a single tree, an id belongs to at most one node.**
Assigning an id that's already taken elsewhere in the tree
removes it from the previous owner:

```
$pepper.id = 'cubeb'    # $cubeb (somewhere else in the tree) loses its id
```

The previous owner's id is set to null. A warning is emitted
via `%chain.warn 'duplicate_node_id'` with the details (the
shared id, the old owner, the new owner). Jasmine catches the
warning automatically — see
[jasmine.md § Automatic warning capture](../jasmine/jasmine.md#automatic-warning-capture).

The warning surfaces the situation but doesn't block the
assignment; the new owner gets the id, the old owner loses it,
and execution continues.

<a id="cross-tree-behavior"></a>
### 5.2 Cross-tree behavior

Uniqueness is **per-tree**, defined as "the connected tree
under this node's root." Nodes in different trees can share
ids freely; a node moved into a tree triggers the uniqueness
...
### 5.4 Subclass override

Subclasses commonly need to sync the id with some other piece
of state — Uma's element class, for example, mirrors the node
id to the HTML `id` attribute on the element. Override the
getter/setter and call `super` to keep the tree-wide uniqueness
behavior:
...
`match?` follows the [`?` suffix convention](../charlie.md#the--suffix):
...
### 9.1 Stopping descent at matches
...
### 13.4 `$node.document` from inside the tree
...
## 15 Bucket discipline: the `uns` slot
...
the class designer wants; `uns` is simply reserved by
convention.

This is a discipline rule, not enforcement. The charlie runtime
