...
{"vibecode": {
	"doc": "dsl",
	"role": "canonical doc on Charlie's DSL architecture — first-class commitment to using the language's own DSL machinery wherever practical",
...
- **Eating our own dog food.** The DSL machinery has to be good enough to power Charlie's own definition. That's the proving ground.

## The four-tier token model

Every word that shows up in Charlie source falls into one of four tiers. The tier determines whether the word is parser-baked, reserved, or DSL.
...
Each of `inherits`, `field`, `accessor` is a bwc resolved through the class-definer's DSL. See [charlie.md § Classes](charlie.md#classes) for the class-DSL command set in detail.
