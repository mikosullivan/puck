...
that prevents arbitrary Charlie code from accessing Lua directly.
...
Charlie-callable surface, and is loaded by the engine at startup —
**not at runtime**. Charlie classes consume bindings the same way
...
engine operator decides what bindings are installed; Charlie code
...
- A deployment's binding surface is a known set, established at
  deploy time.
- Untrusted Charlie code cannot pull in new Lua at runtime.
...
class's Charlie code then calls the binding's exposed surface to
...
3. Charlie class loads; if it depends on a binding, the engine
   checks the binding is present. If not, error.
4. Class methods call the binding's surface as needed; binding
   internals call Lua.
5. Bound Lua functionality is exposed to Charlie only through
...
- **What's the binding API shape?** Probably thin
  Charlie-callable functions and primitive data exchange. Full
