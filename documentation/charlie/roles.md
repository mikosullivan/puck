...
- **`user`** — the role the program's own code runs as. Bootstrap state:
  every Charlie program begins life here unless something explicitly
...
Charlie code. The runtime bails to the engine immediately, and the engine
...
- Anything where the engine specifically wants to ensure no Charlie code
...
external value entering Charlie comes through some faucet, and the
faucet's role is what owns the value.

When Charlie pulls a value through a faucet, the runtime tags that value
