...
- [Statements](#statements)
  ...
  - [Bare Word Command Calls](#bare-word-command-calls)
  - [Operators](#operators)
- [Control Flow](#control-flow)
  ...
  - [Break](#break)
- [Blocks](#blocks)
- [Function and Closure Definitions](#function-and-closure-definitions)
- [Return](#return)
...
- [Source Position Annotations](#source-position-annotations)
  - [What gets annotated](#what-gets-annotated)
  - [CharlieJSON-only origins](#charliejson-only-origins)
  - [Open questions](#open-questions)
- [Known Gaps](#known-gaps)
  - [Hash key order](#hash-key-order)
...
CharlieJSON is a runtime artifact. By convention, code is shared as Charlie source, not
as CharlieJSON.

CharlieJSON also serves as a **canonical semantic intermediate** between source forms.
The current pipeline is Charlie source → CharlieJSON → execution, but the architecture
permits multiple source syntaxes (different parsers fanning in to CharlieJSON) and
multiple pretty-printers (different surface forms fanning out from CharlieJSON). The
project shipped with a single canonical surface (Charlie), but the multi-syntax option
...
The bootstrap parser must be written directly in CharlieJSON, since Charlie cannot parse
...
sugar — it does not expand in CharlieJSON.
...
Charlie equivalent: `$foo.save`

```json
[{"var": "foo"}, "greet", {"name": {"value": "Jean-Luc"}}]
```

Charlie equivalent: `$foo.greet(name: 'Jean-Luc')`
...
### 6.4 Bare Word Command Calls

```json
[{"bwc": "puts"}, {"value": "hello world"}]
```

Charlie equivalent: `puts 'hello world'`
...
Charlie equivalents: `$foo == 'bar'`, `$x + 1`, `$a && $b`
...
Charlie equivalent: `break`

```json
[{"bwc": "break"}, {"value": 2}]
```

Charlie equivalent: `break 2`
...
## 9 Function and Closure Definitions
...
Charlie equivalent: `return $result`
...
Charlie equivalent: `raise 'borg.com/exception/assimilation'`
...
Charlie equivalent: `%chain['user'] = 'picard'`

```json
[{"sys": "chain"}, "get", {"key": {"value": "user"}}]
```

Charlie equivalent: `%chain['user']`

---

<a id="document-statements"></a>
## 13 Document Statements
...
## 14 Source Position Annotations
...
The mechanism: each CharlieJSON node optionally carries a **`line`**
...
- **File identifier alongside line.** Line numbers alone aren't
  ...
  a `file` field at the top of the CharlieJSON program, or inherited
...
regular CharlieJSON statements. The structure mirrors both Charlie source syntax and the
