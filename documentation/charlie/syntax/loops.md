...
[charlie.md § The `as` Keyword](../charlie.md#the-as-keyword);
...
Charlie offers three main looping forms: the condition-driven `while`,
the collection-driven `.each`, and a handful of numeric iteration
helpers (`times`, `upto`, `downto`). Pick the one whose shape matches
the loop you have.
...
[charlie.md § When `do` is Required](../charlie.md#when-do-is-required)
for the rule.
...
- `break N` where N exceeds the number of enclosing loops raises
  `puck.uno/error/invalid_argument`. Use named-loop
...
[Naming a loop with `as`](#naming-a-loop-with-as)). A natural
