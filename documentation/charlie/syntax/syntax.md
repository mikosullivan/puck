...
{"vibecode": {
	"doc": "syntax",
	"role": "umbrella landing page for Charlie syntax topics. Per-construct references that complement the language spec in charlie.md."
}}
~~~

Per-topic syntax references. The big-picture spec lives in
[charlie.md](../charlie.md); this directory breaks it out by
construct so each topic gets its own page.

- **[Assignment operators](assignment-operators.md)** — `=`, `&=`,
  compound forms.
- **[Class definition](../charlie.md#classes)** — `class` /
