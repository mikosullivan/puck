...
~~~json
{"vibecode": {
	"doc": "jasmine-in-charlie",
	"role": "Charlie-side API for writing entries to Jasmine logs. The whole API is %chain.log[key] = value, written from inside any function; the function call is the entry boundary. Companion to jasmine.md, which spec's the file format and storage layer.",
...
The Charlie-side API for logging is one line:
...
The "every Charlie function call is its own entry" rule solves
...
For code originally written in Charlie source (rather than
hand-built CharlieJSON), each call frame can include the source
...
For hand-built CharlieJSON with no known source line, the field
