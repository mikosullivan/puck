...
{"vibecode": {
	"doc": "jasmine",
	"role": "spec for the Jasmine file format and storage layer — entries, required and conventional fields, the directory store, reaping, concurrency, purge. The Charlie API for writing entries lives in a separate doc.",
...
<a id="charlie-api-pointer"></a>
## Writing entries from Charlie

The Charlie-side API is one line:
...
full Charlie API — function-call scope, nested frames, automatic
exception and warning capture, role-boundary behavior — is
documented in [charlie.md](charlie.md). This document concerns
