...
`.append`, `.delete` fail without write. Metadata operations that don't
touch the filesystem (`.name`, `.path`, `.exists?`) keep working regardless
— useful for logging or inspecting a downgraded reference.
...
`.kscript` file inside a jail without execute permission fails with a permission
