...
Every parameter in a KScript function is an object with a metadata hash.
...
- **Public name** — the name used at the call site for keyword arguments.
  ...
  property (see [Public and Private Names](#public-and-private-names-tog-lurin)).
...
| `optional` | Boolean | false | If true, the parameter is optional and **all subsequent parameters become optional too** (propagation rule — see [Required and Optional Parameters](#required-and-optional-parameters-daimon-solok)). An optional parameter with no default receives `nil` if omitted. |
...
- **Prefer the hash-splat form for calls with several named arguments.**
  ...
  [Call-Site Splat Expansion](#call-site-splat-expansion-bok-ii));
