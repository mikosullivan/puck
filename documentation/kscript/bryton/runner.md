...
- **Order** — entries in `files` run in their listed (JSON-object)
  order. So `simple.kscript` runs first, then `details/` (a
...
- **Files and directories listed uniformly** — `details` (a
  subdirectory) sits alongside `simple.kscript` (a file) and is
...
```
dev.scratch.kscript      # ignored
dev.experiment.rb        # ignored
dev.new_idea.kscript     # ignored
```
...
directory listing sees `dev.foo.kscript` and knows "this is
...
`bryton.rb`, `bryton.sh`, `bryton.kscript`).
...
- **Reproducibility.** Running `bryton.kscript` produces the same
...
Each language used for Bryton tests (KScript, Ruby, Python,
