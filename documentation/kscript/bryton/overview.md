...
tool for KScript and is **language-agnostic at the runner level**
...
```bash
./test_foo.kscript            # works
bryton.kscript ./test_foo.kscript   # also works
bryton.kscript path/to/dir    # runs everything in a directory
```
...
[resolution rule](xeme/xeme.md#resolution); the runner walks the
...
- KScript helper library (assertion primitives, personal-config
...
- Per-language libraries beyond KScript (Ruby, Python, JS, etc.
...
- **[Runner](runner.md)** — directory walking, bryton.json,
  ...
  precedence chain.
- **[Icons](xeme/icons/)** — the canonical icon set shipping with
  the Xeme spec.
