...
**Tee is an engine decision, not a KScript-level option.** Scripts
can't choose; the engine operator does, at the same point they
decide whether to grant the handles at all. From inside KScript,
...
**Possible future feature:** a KScript-level tee option on the
...
[`?` suffix convention](../kscript/kscript.md#the--suffix):
