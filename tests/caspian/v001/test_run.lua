--[[
{
  ...
  "verifies": "engine.run reads tests/caspian/v001/fixtures/hello_world.caspj, parses the canonical CaspianJ, dispatches the single statement, and returns a value whose payload == 'hello' to the host"
}
]]
...
runner.test("hello-world fixture returns a string value with payload 'hello'", function()
    local v = engine.run("tests/caspian/v001/fixtures/hello_world.caspj")
...
runner.test("bootstrap state is in place after run completes", function()
    engine.run("tests/caspian/v001/fixtures/hello_world.caspj")
