...
runner.test("raises on a non-string literal payload", function()
    engine.bootstrap()
    local ok = pcall(function()
        engine.materialize({ value = 42 })
    end)
    assert_.is_false(ok, "materialize should raise on a number payload in Aslan")
end)
...
runner.test("raises on an unsupported expression form", function()
    ...
    assert_.is_false(ok, "materialize should raise on {var:...} in Aslan")
end)
