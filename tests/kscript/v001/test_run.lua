--[[
{
  "file": "tests/kscript/v001/test_run.lua",
  "test_id": "T1.7",
  "level": "integration_end_to_end",
  "verifies": "engine.run reads tests/kscript/fixtures/hello_world.ksj, parses the canonical KSJ, dispatches the single statement, and returns a value whose payload == 'hello' to the host"
}
]]
...
runner.test("hello-world fixture returns a string value with payload 'hello'", function()
    local v = engine.run("tests/kscript/fixtures/hello_world.ksj")
...
runner.test("bootstrap state is in place after run completes", function()
    engine.run("tests/kscript/fixtures/hello_world.ksj")
