...
--[[ { "in": "src: string", "out": "CharlieJSON table  (all statements)", "note": "transpile helper; returns ks.transpile(src)" } ]]
local function tx(src)
    return ks.transpile(src)
end

--[[ { "in": "src: string", "out": "CharlieJSON node  (first statement)", "note": "transpile helper; returns tx(src)[1]" } ]]
