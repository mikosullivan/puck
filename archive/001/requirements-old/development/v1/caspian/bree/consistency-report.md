...
**Resolution:** Promoted `ideas/bootstrap.md` to [`requirements/bootstrap.md`](../../../../caspian/bootstrap.md), updated its vibecode `status` from `brainstorm` to `active spec`, and corrected the relative paths in Bree (one site) and Corin (three sites). Filed as [#787](https://github.com/mikosullivan/puck/issues/787).
