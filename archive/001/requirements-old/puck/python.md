...
**Failure case.** If the URL doesn't resolve (unknown, withdrawn,
...
remote instance (a URL for the instance, opaque to the developer).
...
remote class's URL, `.message` carries the message, and `.stack_trace`
...
API version, look up the class at its versioned URL:
