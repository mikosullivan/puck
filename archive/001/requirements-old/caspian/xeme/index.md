...
{"vibecode": {
	"doc": "xeme",
	"role": "spec for Xeme, the JSON-shaped tree format for reporting structured results (test outcomes and other tree-shaped records); covers the required success field, groups and leaves",
...
outcomes. It is the format Bryton consumes and produces.
...
Correct — a leaf may carry `errors`:
...
```json
{
    ...
    "errors": [
        {"class": "bryton/assertion", "message": "expected 'foo@bar', got 'foo'"}
    ]
}
```

Correct — a leaf may carry `nulls`:

```json
{
    "success": null,
    "name": "test_email_lookup",
    "nulls": [
        {"class": "bryton/timeout", "message": "exceeded 5s deadline"}
    ]
}
```

Correct — a group has neither `errors` nor `nulls`:
...
```json
{
    ...
    "name": "email_suite",
    "nested": [
        {"success": true,  "name": "test_email_format"},
        {"success": false, "name": "test_email_lookup"}
    ]
}
```

Incorrect — a group with `errors` triggers a validator warning:
...
```json
{
    ...
    "errors": [
        {"class": "bryton/mistake", "message": "errors on a group"}
    ]
}
```

Incorrect — a group with `nulls` triggers a validator warning:
...
```json
{
    ...
    "nested": [
        {"success": true, "name": "test_email_format"},
        {"success": null, "name": "test_email_lookup"}
    ],
    "nulls": [
        {"class": "bryton/mistake", "message": "nulls on a group"}
    ]
}
```

In both incorrect cases, the Xeme is still passed through — the validator's warning is the correction signal, not a rejection.
...
own success and children, the resolution rule becomes "min over
own success and all children's successes." That introduces a
parent-null edge case (a parent's own `null` interacting with
all-`true` children under strict least-good ordering), which is
**not yet resolved**.
...
Working convention: **URL-style identifier without the domain
prefix.** Examples:
...
[URL](../../../../../ideas/uns/) (slash-separated segments, lowercase, etc.).
Third-party Xeme producers can use their own URL prefix:
...
[`icons/`](https://github.com/mikosullivan/puck/tree/main/documentation/caspian/packages/bryton/xeme/icons/). Each Xeme `class` maps to an SVG (or GIF) at
    ...
    "nested": [
        {
            ...
            "meta": {"name": "auth_test.casp", "run_time": 0.4}
        },
        {
            "success": false,
            "class": "bryton/group/file",
            "meta": {"name": "db_test.casp", "run_time": 1.2},
            "nested": [
                {
                    "success": false,
...
```json
{
    ...
    "meta": {"name": "expected_test_that_doesnt_exist.casp"}
}
```
...
- **`success: true` is a valid Xeme** — the minimum-content leaf `{"success": true}` validates and represents a definitive pass.
- **`success: false` is a valid Xeme** — `{"success": false}` validates and represents a definitive fail.
- **`success: null` is a valid Xeme** — `{"success": null}` validates and represents an undetermined verdict.
- **Xeme without `success` fails validation** — `{}` and `{"name": "x"}` both raise; `success` is required.
- **`success` values outside {`true`, `false`, `null`} raise** — `{"success": 1}`, `{"success": "yes"}`, `{"success": "true"}` all fail validation.
- **A leaf has no `nested` field** — an object without `nested` is a leaf regardless of any other field.
- **A group has `nested`, even if empty** — `{"success": true, "nested": []}` is a group; `{"success": true}` is a leaf.
- **A group carrying `errors` produces a validator warning but passes through** — the Xeme is not rejected; the warning surfaces the inconsistency.
- **A group carrying `nulls` produces a validator warning but passes through** — same posture.
- **A leaf carrying `errors` validates cleanly** — `errors` is leaf-appropriate.
- **A leaf carrying `nulls` validates cleanly** — `nulls` is leaf-appropriate.
- **A group with neither `errors` nor `nulls` validates cleanly** — the normal shape.
- **Resolution: leaf's `success` is unchanged** — `{"success": true}` stays `true` after resolution.
- **Resolution: group with `own: null` and all-`true` children resolves to `true`** — adopts children's verdict.
- **Resolution: group with `own: null` and one `false` child resolves to `false`** — least-good rule.
- **Resolution: group with `own: null` and one `null` child (no `false`) resolves to `null`** — least-good rule.
- **Resolution: group with `own: true` and one `false` child resolves to `false`** — parent can be less successful than the children's verdict.
- **Resolution: group with `own: false` and all-`true` children resolves to `false`** — the explicit-fail case.
- **Resolution: group with `own: false` and mixed children resolves to `false`** — `min(own, children)` is still `false`.
- **Resolution: empty-`nested` group with `own: true` resolves to `true`** — no children means no children's verdict to combine.
- **Resolution: empty-`nested` group with `own: null` resolves to `null`** — nothing to defer to.
- **Resolution ordering is `false < null < true`** — pairwise checks against every ordered combination.
- **Resolution excludes children with `meta.skipped: true`** — a group whose only failing child is skipped resolves the same as if the child weren't there.
- **Resolution: group of all-skipped children with `own: null` resolves to `null`** — no unskipped children to combine.
- **Resolution: group of all-skipped children with `own: true` resolves to `true`** — own verdict stands.
- **Resolution: `meta.skipped: false` participates normally** — the flag has to be truthy to exclude.
- **Explicit fail on a group does not warn** — `{"success": false, "nested": [{"success": true}]}` is allowed; no nanny.
- **Group's resolved `success` never exceeds any unskipped child's `success`** — property-based check.
- **Reserved fields are all optional** — `{"success": true}` alone is valid; no reserved field is required.
- **`meta.name` accepts a string** — human-readable identifier.
- **`meta.uuid` accepts a string** — UUIDv4 form recommended but not enforced.
- **`meta.timestamp` accepts an ISO-8601 millisecond-precision string** — e.g., `"2026-05-13T18:42:00.123Z"`.
- **`meta.run_time` accepts a number** — seconds; fractional accepted.
- **`meta.skipped` accepts a boolean** — nonboolean values are outside the convention.
- **`io.stdout` accepts a string** — captured stdout stream.
- **`io.stderr` accepts a string** — captured stderr stream.
- **`location.file` accepts a string** — path is producer's choice (absolute or relative).
- **`location.line` accepts an integer** — line number.
- **`location.stack` accepts language-specific content** — no shape enforced by Xeme.
- **`location.tags` accepts a hash of tag name to opaque value** — no schema on values.
- **`location.tags` is per-node, not inherited** — a child's `location.tags` doesn't automatically include the parent's tags.
- **`location.tags` with falsy values are omitted in producer output** — `{"integration": false}` should not appear; producers write the effective set.
- **Top-level `tags` is distinct from `location.tags`** — top-level describes the run's result; location describes the source.
- **Top-level `tags` with falsy values are omitted** — same posture as `location.tags`.
- **`errors` entries are flat objects with a `class` field** — no `nested` inside an entry.
- **`errors` entries with additional fields are accepted** — `message`, `details`, etc. are producer-namespaced.
- **`nulls` entries are flat objects with a `class` field** — same shape.
- **`errors` and `nulls` are arrays, not single objects** — one entry per cause; multiple independent causes are multiple entries.
- **`class` accepts a URL-style identifier without the domain prefix** — `"bryton/group/dir/runner"`.
- **`class` accepts a third-party URL form** — `"myorg.com/test/integration"`.
- **Empty arrays and empty hashes are omitted, not written as placeholders** — producer-side rule; a valid Xeme should not carry `"warnings": []` when there are no warnings.
- **Producers can add non-reserved top-level fields** — `{"success": true, "exit_code": 0}` is valid; `exit_code` is producer-namespaced.
- **The reserved set is closed** — adding a top-level field with a reserved name and a wrong shape (e.g., `"nested": "string"`) fails validation.
- **`nested` must be an array of Xemes** — each element itself must satisfy Xeme validation.
- **A group nested to arbitrary depth resolves correctly** — recursive resolution walks the full tree.
- **`misc` is a pass-through hash** — consumers should not interpret it; validators leave it intact.
- **`corporate` is a pass-through hash** — same posture.
- **`vibecode` is a pass-through hash** — consumed by AI tooling, not by Xeme processors.
- **UUID auto-generation is producer-optional** — Xemes without UUIDs remain valid.
- **`meta.timestamp` typically appears only at the top level** — nested Xemes may omit it and inherit by convention; validators do not require it on every node.
- **`bryton/null/promise` in `nulls` propagates as `null` up the tree** — no ancestor of a `promise` result can resolve to `true`.
- **Trimming: a `success: true` leaf is removed** — `{"success": true}` inside a group's `nested` array disappears after trim.
- **Trimming: a `success: false` leaf is kept** — never removed.
- **Trimming: a `success: null` leaf is kept** — null-verdict leaves are informational.
- **Trimming: a group whose children all trim away and whose `success` is `true` is removed** — cascade upward.
- **Trimming: a group whose children all trim away but `success` is `false` is kept without `nested`** — the explicit-fail group survives.
- **Trimming: a group whose children all trim away but `success` is `null` is kept without `nested`** — same.
- **Trimming a Xeme with no failures collapses to `{"success": true}`** — the whole-tree-passed case.
- **Trimming preserves every retained value verbatim** — no `success` value is rewritten during trim.
- **Trimming preserves reserved-field values on retained nodes** — `errors`, `meta`, `location`, etc. remain intact.
- **Trimming an already-trimmed tree is idempotent** — running trim twice produces the same result as running it once.
- **Trimming works bottom-up** — a leaf's status determines its own fate before the group above it is evaluated.
- **Icons: class-lookup walks up the class path** — `"group/file/rb"` tries `group/file/rb.svg`, then `group/file.svg`, then `group.svg`, then `generic.svg`.
- **Icons: failure-cause lookup walks under `results/`** — not `tests/`.
- **Icons: missing specific icon falls back to sensible parent automatically** — first match wins.
- **Non-UTF-8 bytes in `io.stdout` / `io.stderr` are still accepted as strings** — captured I/O may include binary; producer's responsibility to encode.
- **Very large `nested` arrays resolve correctly** — no artificial ceiling on breadth.
- **Deeply-nested trees resolve without stack overflow** — implementation-side robustness check.
- **`success` at the top level equals the resolved verdict of the whole tree** — recursive property after resolution.
- **Xeme JSON round-trips through `JSON.parse(JSON.stringify(x))` cleanly** — pure-data format.
- **A producer that emits both a group's `success` and its children's `success` values is honored: `min(own, children)`** — the least-good rule again, stated as a Xeme-consumer contract.
- **Mixed-node parent-null edge case is spec-open (TBD)** — the settled rule (once decided) determines whether a mixed node with `own: null` and all-`true` children resolves to `true` or `null`; consumers using mixed nodes should treat this case as open until the spec pins it down.
