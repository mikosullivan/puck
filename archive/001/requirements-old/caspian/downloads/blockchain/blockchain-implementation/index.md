...
{"vibecode": {
	"doc": "blockchain",
	"role": "design spec for Puck's object-signing and blockchain registry: how URL-addressed objects prove provenance and integrity",
...
The Puck blockchain will hold signatures of Caspian libraries. You
can use those signatures to verify the provenance and timestamp of a
library. You can also use the blockchain to find libraries within
a given timespan.
...
blockchain. Caspian will natively know about that service. We won't
...
its URL address at a specific time and signed by a specific key.
...
organisation, or a company running internal Caspian infrastructure
...
3. Puck posts an `endorse` block with `endorsement: "provenance"`, embedding a signature
   in the endorsement entry.
...
`semver` is optional. When present, it must be a valid [SemVer 2.0](https://semver.org) string and is used by the chain's duplicate-check rule (see [endorse blocks](#authority-blocks-1) below) and by resolvers that select libraries by semver. Publishers that don't version with semver simply omit the field; date-pinning still works.
...
- `intent` — the block's intent; well-known values: `authority`, `grammar`, `endorse`,
  `delegate`, `deprecate`, `revoke`. **Block 1 must be `authority` and block 2 must be `grammar`**; see [Bootstrap blocks](#bootstrap-blocks).
- `grammar` — reference to the grammar block (`{"hash": "...", "version": "..."}`); required on every block **except** the bootstrap pair (positions 1 and 2)
...
- `signer` — URL address of the signing entity
...
### Bootstrap blocks (positions 1 and 2)

The chain begins with two specific blocks, in order:

- **Block 1** — Puck.uno's **authority** block. The anchor of trust; carries the baked-in public key plus `puck_primer` and `vibecode`. Block 1 has **no `grammar` field**; it is the bootstrap exception. It is understood to implement the grammar defined in block 2.
- **Block 2** — the first **grammar** block. Defines the format and validation rules used by block 1 and any subsequent block that references it. Block 2 itself also has no `grammar` field (it *is* the grammar), or self-references its own `record_hash` once that hash is determined — either is allowed and equivalent.

From block 3 onward, every block must carry a `grammar` field per the [Grammar](#grammar) section below.
...
Every block **except the [bootstrap pair](#bootstrap-blocks) (positions 1 and 2)** must include a `grammar` field in its payload referencing a grammar block by hash and version string:
...
- `grammar` — reference to the grammar block. **Omitted on the chain's first authority block** (block 1), which is understood to implement the grammar defined in block 2. Required on every subsequent authority block (anyone posting a new authority on the chain references whatever grammar is current at that time).
...
- `uns` — URL address of the software being endorsed (when applicable)
- `semver` — **optional**. SemVer string for the published artifact (e.g. `"2.1.0"`).
  When present, must be a valid [SemVer 2.0](https://semver.org) string. Subject to the
  [duplicate-check rule](#semver-duplicate-check) below. Publishers that don't version
  with semver omit this field; date-pinning continues to work without it.
...
- `endorsements` — array of claims; each entry is a flat object whose fields are:
  - `endorsement` — what is being claimed; open-ended string; well-known values:
    ...
    `provenance` (fetched from the stated URL at the stated time),
  ...
  - artifact fields — for `provenance` entries, the artifact's metadata (`name`,
    `description`, `language`, `license`, `semver`, `artifact_hash`, `artifact_url`,
    etc.) appears directly in the entry alongside `endorsement`. `artifact_hash` and
    `license` are required; `semver` is optional. Software bodies are not stored on the
    chain — `artifact_hash` is the cryptographic anchor and the artifact itself lives
    off-chain at `artifact_url` or another resolvable location.
...
**Semver duplicate check.** When an `endorse` block carries `semver`, the chain rejects
the block if another endorsement with the same `uns` + `semver` combination is already
on record. Republishing an existing semver would create ambiguity for resolvers that
select by semver — consumers asking for `borg.com/parser` at `2.1.0` should get exactly
one answer. To fix a buggy publication, bump the semver and post a new endorsement.

This is distinct from the [Submit endpoint's content-identity idempotency](#submit-domain-owner-puck):
identical bytes posted twice are silently coalesced; *different* bytes posted under the
same `uns` + `semver` are rejected.

The chain does **not** enforce monotonic semver progression. A `1.2.5` patch posted
after `2.0.0` is legitimate (parallel-branch publishing), as are pre-release tags and
out-of-order historical posts. The only rule is uniqueness within a URL.
...
- `entity` — URL address of the entity being trusted
...
**deprecate** — marks a URL or semver as deprecated. Does not invalidate the record;
...
The HTTPS API for the blockchain — endpoints, request/response shapes, idempotency rules, and the rest — lives in the sibling [`api/`](../api/) directory. This doc covers the chain itself (design, signing, authority blocks, trust tiers); the API spec covers how clients talk to it.
...
the call chain. The general model is documented in [versioning](../versioning/);
...
Each object may declare its own dependencies — by URL name — along with an optional
...
The broader opportunity is significant. The Puck blockchain is not limited to Caspian
...
**Block ordering invariant.** Every rebuild must produce the [bootstrap pair](#bootstrap-blocks) at positions 1 and 2 — block 1 is Puck.uno's authority block (with no `grammar` field), block 2 is the first grammar block. The rebuild tooling enforces this; if any post script or producer ever emits blocks in a different order, the rebuild fails before signing.

**When rebuilding the chain after a field-name or shape change** (e.g. the `version` → `semver` rename for library semver), **double-check that every consumer of the chain still supports the new and changed keys** before running the rebuild. Touch-points include:

- The [Lua sim](lua/) — `chain.lua`, `sign.lua`, `util.lua`, `api/object.lua`, `api/request.lua` may reference the old field names.
- The [post-* shell scripts](.) — `post-grammar.sh`, `post-registry-entries.sh`, `post-horatius-root.sh` — produce chain entries and may hardcode field names in their generated JSON. They also determine block order — confirm they emit authority first, grammar second.
- Any external producers (puck.uno service code, publisher tooling) that submit blocks.
