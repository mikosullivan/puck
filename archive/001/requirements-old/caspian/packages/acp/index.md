...
- **Async dispatch** — leverages the `$request.promise` pattern from the [Puckai agent design](https://puck.uno/documentation/requirements/packages/agent/#request-promise); waits for Caspian's broader async machinery to firm up.
...
- **`log: true`** — side effect; writes the worldlet to the default [Jasmine log](https://puck.uno/documentation/requirements/packages/jasmine/). Return value is unchanged — still the decision body unless `audit: true` is also set.
...
- [`puck.uno/ai/agent`](https://puck.uno/documentation/requirements/packages/agent/) — the established Caspian pattern for outbound agent calls; the ACP client shape borrows the `.new(url:) + .send(...)` surface from it.
- [`%net.http_client`](https://puck.uno/documentation/requirements/network/http/client/) — the HTTP layer ACP rides on.
