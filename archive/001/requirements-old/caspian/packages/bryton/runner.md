...
- **Order** — entries in `files` run in their listed (JSON-object)
  order. So `simple.casp` runs first, then `details/` (a
...
- **Files and directories listed uniformly** — `details` (a
  subdirectory) sits alongside `simple.casp` (a file) and is
...
```
dev.scratch.casp      # ignored
dev.experiment.rb        # ignored
dev.new_idea.casp     # ignored
```
...
directory listing sees `dev.foo.casp` and knows "this is
...
`bryton.rb`, `bryton.sh`, `bryton.casp`).
...
- **Reproducibility.** Running `bryton.casp` produces the same
...
Each language used for Bryton tests (Caspian, Ruby, Python,
