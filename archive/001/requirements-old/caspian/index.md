...
A class is defined with the `class` keyword and a URL name. The block contains schema
...
The URL name may be omitted. A bare `class ... end` block produces an
**anonymous class** — a class with no URL identity of its own. The class
...
*location* rather than its URL — e.g., Robinson page files (identified
...
|---|---|
| `inherits 'URL'` | Inherit from a parent class |
...
Built-in type names are strings — `:string` and `'string'` are identical. URL names use
