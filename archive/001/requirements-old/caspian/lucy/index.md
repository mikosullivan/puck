...
[`?` suffix convention](../index.md#the-suffix): falsey on the happy path,
...
|---|---|---|
| `class` | string (URL) | The owning role / class of the function-object whose call frame this is. For top-level scripts, the engine fills in a synthetic URL (e.g., `puck.uno/script/<name>`). |
...
**URL naming is flat; inheritance is declared.** The class names above
...
prefix in the URL. Their relationship to `puck.uno/exception` is by
**declared inheritance**, not by URL path. Catch behavior, subclass
matching, and the abstract-vs-concrete distinction all follow declared
inheritance — not URL-prefix matching. (See
[ideas/uns/](../../../ideas/uns/) for the general rule that URL
...
`puck.uno/exception` — URL placement and inheritance are independent.
...
- **`as $name` is currently described in [caspian.md § The `as`
  Keyword](../index.md#the-as-keyword) as its own
...
{"vibecode": {
	...
	"puck_namespace": "%puck[URL] to access registered objects",
...
`%puck[URL]` returns the object registered at that URL address — which may be a class,
...
See [mikobase.md](../../mikobase/index.md) for the hot record design.
