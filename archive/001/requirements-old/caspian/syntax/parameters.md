...
| `classes` | Array of strings | nil | Accepted types or URL class names. If set, passing a value of a non-matching type raises an error. Multiple entries mean any of the listed types are accepted. |
...
strings; user-defined class names use the full URL address:
...
- Should `classes` accept a mix of built-in type names and URL addresses
  in the same array?
