...
This is the same property that lets the [roles system](https://puck.uno/documentation/requirements/roles) reason about cross-role calls cleanly: when a function is invoked across a role boundary, the engine knows exactly what the function can see — its parameters and nothing else.

## See also

- [Modules](https://puck.uno/documentation/requirements/modules) — how mutual function references work when functions can't see each other (modules provide a shared object for method-style calls).
- [Caspian roles](https://puck.uno/documentation/requirements/roles) — why closed function scope is important for the security model.
- [Parameters](https://puck.uno/documentation/requirements/syntax/parameters) — declaring what a function takes in.
