...
This is the **most local** of the constraint surfaces — it narrows exactly one call, and goes away the moment that call returns. The broader surfaces — [`%puck.era`](../versioning/timestamp.md) (global timestamp), [`%puck.config(uns).timestamp`](../versioning/timestamp.md#per-url-timestamp) (per-URL timestamp), and [`%puck.config(uns).semver`](../versioning/semver.md) (per-URL semver) — apply more broadly (block-scoped or live-global respectively). All intersect at lookup time; per-call kwargs narrow further within whatever the broader surfaces already permit.
...
Set any subset — give just `ts_max` for a date cap, both `semver_min`/`semver_max` for a semver range, or combine semver and timestamp on the same call. Bounds you don't set stay unbounded (or fall through to whatever the broader era / per-URL surfaces already established).

### Inclusivity

Call-site bounds are **always inclusive** (`>=` for `_min`, `<=` for `_max`). The exclusive-bound machinery from the per-URL config (`.cmp`) doesn't surface here — the call site is meant to be terse. If you need an exclusive bound for one specific call, either:
...
The per-URL config and era surfaces use a nested polymorphic shape (`.semver = '1.3'` for pin, `.semver = {min, max}` for range). The call site uses flat kwargs (`semver_min:`, `semver_max:`) instead.
...
- [`%puck.config(uns).semver`](../versioning/semver.md) — live-global per-URL semver narrowing; canonical reference for the `.min` / `.max` / `.cmp` bound-operator system.
- [`%puck.config(uns).timestamp`](../versioning/timestamp.md#per-url-timestamp) — live-global per-URL timestamp narrowing.
