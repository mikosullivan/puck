...
{"vibecode": {
	"doc": "touchstone",
	"role": "spec for puck.uno/touchstone, the base HTTP server class inherited by Sammy and by any other HTTP middleware built on Puck; holds shared infrastructure (content-type defaults, Jasmine integration, common HTTP plumbing) but is not directly instantiable",
...
[Sammy](../sammy/index.md) inherits from, and that any other HTTP middleware
built on Puck will inherit from too. Touchstone is **not directly
instantiable as a working server.** It holds the shared infrastructure
its descendants need (content-type factory defaults,
[Jasmine](../../../../packages/jasmine/index.md) integration, common HTTP plumbing)
but doesn't itself decide *how* to serve content. That decision is
each descendant's job.

In practice, Caspian code reaches for a Touchstone descendant rather
than Touchstone itself. [Sammy](../sammy/index.md) ships with Puck and covers
the small-sites / microservices / single-file apps use case.

| Class | URL | Use case |
|---|---|---|
| **Touchstone** | `puck.uno/touchstone` | Base class. Content-type defaults, Jasmine integration, shared HTTP plumbing. Inherited by Sammy and by any other HTTP middleware built on Puck; not for direct instantiation. |
| **Sammy** | `puck.uno/sammy` | Small sites, microservices, single-file apps. Route handlers as closures, Ruby-Sinatra-style. |

```caspian
# A simple Sammy-style app
...
Spec in development. The shape fills in as Sammy surfaces its
requirements — Touchstone is where shared behavior crystallizes
once a real descendant needs it.
...
`.json` → `application/json`, and so on). Sammy uses this map
when serving files; developers can override per-server.

### Jasmine integration

Touchstone wires up [Jasmine](../../../../packages/jasmine/index.md) logging. The ambient
...
storage; in v1, that means a [directory jail](../../../../built-in-classes/filesystem.md)
...
cookie. Reading and writing behaves like an ordinary Caspian
...
live in core. [Sammy](../sammy/index.md)'s path-selector registrations
build on this same chain mechanism.
...
chain — Sammy path selectors, CSRF guard, CORS, etc. —
implements the same interface; the dispatcher
...
`@start_time`. No new mechanism — the standard Caspian object
model carries it.
...
the developer's custom Handler's URL, etc.) and its position in
`$server.handlers`. Subsystems that compose multiple handlers into
sub-chains include enough detail to identify the inner Handler too.

This information is then surfaced by descendant frameworks in
their admin error displays.
...
The default Caspian behavior would let the later error shadow
...
- **`$headers`** — **raw HTTP header hash.** Keys are canonical
  ...
  **Duplicate-cookie-name warning.** Before serializing,
  ...
  [jasmine/caspian.md § Automatic warning capture](../../../../packages/jasmine/caspian.md#automatic-warning-capture)).
...
- The framework catches the redirect exception at the top of
  ...
  Location header).
- Jasmine entries are scoped to function calls, so the entry
  for the request handler is preserved with whatever was
  written before the redirect unwinds.
...
the mechanism; Sammy inherits it unchanged, and any future HTTP
middleware that descends from Touchstone gets it too.
...
Developers write plain strings (`'self'` as a Caspian string
...
Static file serving is **not** a Touchstone responsibility. Each
descendant handles static files in its own way — their needs
differ enough that a shared implementation in the base class
would compromise them. Touchstone provides the *facts* (the
content-type map) but leaves the *behavior* to descendants.

If you want a server, instantiate [Sammy](../sammy/index.md) — not
Touchstone directly.
