...
When user code enters a `%role.delegate_to(X) do ... end` block (see [roles spec § Frame-scoped delegations](https://puck.uno/documentation/requirements/roles#role-delegate_to)), the engine pushes a frame whose `delegations` field records which role is being granted permissions in this block's scope. The delegation lives on the frame; when the frame is popped (normal exit, alarm unwind, anything else), the delegation goes with it.
...
Puck URL sources use `{"uns": "..."}`. **For multi-file Puck
libraries, each source file gets its own registry entry** with
the file name in the URL path (e.g., `{"uns":
...
- **Multi-file Puck libraries get one srcs entry per file**, with
  the file name in the URL value (e.g., `{"uns":
...
**Multi-file libraries: one entry per file.** A URL-loaded library
...
- **`roles` registry has three entries.** Two engine-bootstrap
  ...
  metadata: where it was loaded from (the URL), where in user
...
- **`srcs` registry mixes local and remote, with tagged kinds.**
  ...
  don't have to parse strings to tell file paths from URL
...
- **The library's role name IS its URL.** `markdown.uno/render`
  as a role name is fine — names are arbitrary strings, and URL
