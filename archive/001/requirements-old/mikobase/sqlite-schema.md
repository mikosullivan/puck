...
- An instance record's class identity lives in the `stack` column (a JSON object of platters); class names inside it are URL strings. There is no foreign key to `records` — the engine validates class existence at write time.
- A class-definition record uses the whole-hash form (`whole_hash = 1`, definition object in `body`) rather than `bucket`+`stack`. Its `body.class` is `puck.uno/class` and `body.name` is the URL being defined.
