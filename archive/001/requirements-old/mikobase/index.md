...
|---|---|---|
| `class`    | every field | The type. Either a primitive (`string`, `number`, `array`, `hash`, etc.) or a URL class name (`foo.com/widget`). |
...
For fields whose `class` is already a URL class name (e.g., `"class": "foo.com/widget"`), the class itself is the type constraint — `items` only applies when the outer `class` is a collection.
...
The file format is specified in [worldlet.md](worldlets/index.md).
...
Class names inside a worldlet use URL — the publisher's domain provides a
