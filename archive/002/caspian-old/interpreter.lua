--[[
{
  "module": "caspian.interpreter",
  "role": "CaspianJ executor — walks a CaspianJ program table and evaluates it",
  "exports": {
    "new":  "(env?) → interpreter  create a new interpreter instance with optional env overrides",
    "exec": "(interp, program)     execute a CaspianJ program (array of statements) in order"
  },
  "env_keys": {
  ...
  "format_note": "handles the transpiler's actual output format, which predates caspianj.md; the two will be reconciled in a later pass",
...
--[[
{
  ...
  "role": "each Caspian primitive type has its own method table; dispatch in exec_stmt looks up the receiver's Lua type here",
  "types": {
    "String":  "string operations; + is concatenation, not addition",
    "Number":  "arithmetic and comparison",
    "Boolean": "equality only"
  },
  "extension": "user-defined Caspian classes will register their method tables through a separate class registry (not yet implemented)"
}
]]
...
--[[ maps Lua type() strings to their Caspian method table ]]
local type_methods = {
...
--[[
{
  "in": "expr: any CaspianJ expression node (table) or raw Lua scalar",
...
--[[
{
  "in": "stmt: one CaspianJ statement — either an array or a no-op object",
...
--[[
{
  ...
  "note": "also accepts 'branches' as an alias for 'expressions' to support both transpiler and caspianj.md formats"
}
]]
...
--[[
{
  "in": "program: CaspianJ array of statements (the top-level program node)",
  "out": "nil",
