...
A companion to [mcp-and-caspian](https://puck.uno/ideas/mcp-and-caspian), which asks "how does Caspian consume MCP so its own agents get external tools?" This doc asks the inverse question: **how could MCP shorten the ramp for an AI coding agent that opens a Caspian project and knows nothing about Caspian?**
...
- Every `requirements/**/*.md` page, addressed by URL (see [URL symmetry](#url-symmetry-with-caspians-library-model) below).
- Every `ideas/**/*.md` page, likewise.
...
- **Downside.** Install friction. The reason [ideas/lsp](https://puck.uno/ideas/lsp/) was deferred is that requiring a local install conflicts with the first-contact goal. Same objection applies here.
...
- The **LSP** ([ideas/lsp](https://puck.uno/ideas/lsp/)) serves editor UI — diagnostics, hover, completion. Human-facing. Deferred post-V1.
- The **MCP-for-agents server** (this doc) serves an AI agent's mental model — resources, tools, prompts. Agent-facing.
- The **MCP-as-client work** ([ideas/mcp-and-caspian](https://puck.uno/ideas/mcp-and-caspian)) lets Caspian's own `$agent.yield` agents consume external MCP servers. Independent direction.
