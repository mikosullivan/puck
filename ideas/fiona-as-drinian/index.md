...
{"vibecode": {
	...
	"role": "spitball space for using Fiona (SQLite-backed object store, split out to mikosullivan/fiona 2026-08-03) as the storage layer for all of CVM — Caspian's runtime state. Not committed to; V1 CVM is the in-memory hash approach per requirements/cvm/. This directory is a reference for Fiona's current table shapes so we can brainstorm without opening the other repo every time.",
...
- **Trees present.** Cost per write is O(sum of tree sizes). For CVM's roles tree (10-50 nodes, one tree), microseconds per write.
...
None of these are needed for CVM's scale. The starting shape above is good enough; the room for a much better plan is real but not the current priority.
