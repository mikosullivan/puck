...
Transactions with fault protection start to look like the primitives Mikobase would need for durable session semantics — atomic writes, rollback on failure, data-typed outcomes to inspect. If CVM and Mikobase share the storage substrate (per [[project_caspian_mikobase_shared_class_definitions]]), transactions here may become transactions there.

For now: track this as a CVM-runtime feature. Note the overlap; consolidate when the Mikobase spec gets serious.
