...
**A captured chain surface stays usable after the grant that made it reachable ends.** From [roles/object-access § Chain-reachable surfaces are first-class values](https://puck.uno/requirements/roles/object-access#chain-reachable-surfaces-are-first-class-values): the grant on `%chain.X` controls **permission to call methods on `%chain`**. Once the method has been called and returned an object, the object is a normal held reference — it's usable as long as anyone holds it, regardless of what the chain looks like later.
...
- [roles/object-access](https://puck.uno/requirements/roles/object-access) — the V1 rules this decision composed from.
- The [chain grant model](https://puck.uno/requirements/chain/#grant-and-revoke) — the block-scoped grant mechanism whose lifetime story this idea revisits.
