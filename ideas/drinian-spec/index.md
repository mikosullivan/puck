...
~~~vibecode
{"vibecode": {
	"doc": "ideas_drinian_spec",
	"role": "working notes for the Drinian state hash's exact shape as we build it out — companion to requirements/drinian/ which owns the settled spec. Landing pad for design questions surfaced during implementation (roles-tree bootstrap, ast referencing, sequence-counter starting value, primitive-slot location, src on identity objects, etc.) before they get promoted to requirements.",
...
Companion to [requirements/drinian/](https://www.puck.uno/requirements/drinian/). Captures the design decisions we're making as we build the Drinian machinery, before they're settled enough to move into `requirements/`.

## The state hash

`state.new()` (in [src/engine/state.lua](../../src/engine/state.lua)) returns the Drinian state hash — the single top-level table every field of Caspian's execution state lives inside. V1.0 in-memory shape:
...
The `asts` field is the top-level hash the [requirements § The AST lives in Drinian](https://www.puck.uno/requirements/drinian/#the-ast-lives-in-drinian) section left TBD-named. Named `asts` here — plural of ast, matches the `srcs` naming pattern.
...
The counter is stored as a decimal string so it grows indefinitely without bigint machinery, per [references § Object IDs](https://www.puck.uno/requirements/drinian/references#object-ids). The internal increment routine walks digits right-to-left, carries on 9s, and grows the string when the whole thing was 9s — no `tonumber` / `tostring` round-trip, so the counter grows past `2^53` (Lua 5.4's integer-vs-float boundary) with no precision loss.

Nested-object platter IDs use UUIDs instead ([objects § Object IDs](https://www.puck.uno/requirements/drinian/objects#object-ids)) — the Sequence is only for slots where the ID isn't user-visible.
