...
Inverse pairing with [Excalibur](https://puck.uno/ideas/excalibur/excalibur) — Excalibur embeds `.casp` code (and its output) into `.md` files; this pattern embeds documentation into `.casp` files. Same idea, opposite directions. Together they let an author lean either way without losing the other.
