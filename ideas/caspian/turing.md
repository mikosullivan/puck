...
- **Recursion.** Functions and methods can call themselves or each other. The dispatch model in [cvm.md](../../requirements/cvm/sqlite/index.md) imposes no fixed recursion depth — every call pushes a frame onto `engine.state.call_stack`, and the stack grows until something terminates the program (an exception, a `%timeout`, or running out of host process memory). That is exactly the unboundedness the requirement asks for.
