...
- **`.execute`** on file and dir objects — spawns a subprocess with the user's OS privileges. See [linux-support](https://puck.uno/requirements/linux-support/#executing-from-a-directory).
- **`.cd()`** on dir objects — mutates the process's current working directory (in both its permanent and block-scoped forms). See [dirs § .cd()](https://puck.uno/requirements/filesystem/dirs/#cd).
...
- [concepts § Security](https://puck.uno/requirements/concepts#security) — where holding-is-access is stated.
- [concepts § No nanny code](https://puck.uno/requirements/concepts#no-nanny-code) — the general anti-nanny principle that this class of methods deliberately breaks.
- [dirs § .cd()](https://puck.uno/requirements/filesystem/dirs/#cd) — one of the two current nanny methods.
- [linux-support § Executing from a directory](https://puck.uno/requirements/linux-support/#executing-from-a-directory) — the other current nanny method.
