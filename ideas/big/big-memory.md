...
If CVM ever becomes Fiona-backed (or any-DB-backed), every value it holds has to be serializable — either directly (scalars, hashes, arrays, refs to other things in CVM) or via some convention that keeps non-serializable state elsewhere. The current spec is silent on the specifics. This is an open design question that has to get answered before the Fiona swap can happen. Spitballing.
...
A different shape from the previous conversation: **never let a non-serializable value into CVM in the first place.**

Under this model:

- Every value that lives in CVM is serializable, by construction. No exceptions.
- Native resources live in a host-engine sidecar (a Lua-side table, or wherever the host keeps its own state) keyed by IDs.
- Caspian classes that need a native resource hold an ID into the sidecar, not the raw resource. The ID is a plain string; the sidecar isn't in CVM.
...
- **Serialization becomes trivial.** Every value already IS serializable; the serializer never encounters a non-serializable thing because none exist in CVM.
- **Revive is harder.** The IDs in the revived CVM point at sidecar entries that don't exist yet. Either the class's `on_revive` hook re-establishes the sidecar entry from a stored spec, or the wrapper raises on first use ("this handle didn't survive revival — reconnect explicitly").
...
- **Swap-to-Fiona-backing** (in-process, live migration). References to raw handles might still be reachable from Lua-side code that manages them; the CVM-visible state is what changes storage. Silent placeholder plus a sidecar mapping might be acceptable.
...
- **Prevention as the primary discipline.** Native resources always come wrapped; IDs live in CVM; raw handles never do. This makes the common case invisible — CVM is trivially serializable because it never holds a raw handle to begin with.
...
The wrapper discipline is the load-bearing part. Get it right at the API boundary and the rest is small. Get it wrong and every future context (snapshot, Fiona swap, distributed CVM, big-process overflow) has to reinvent the escape hatch.
