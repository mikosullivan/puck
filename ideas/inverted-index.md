...
**Serialization overrides.** `.serialize` outputs the forward hash's content only; `.reconstruct` rebuilds the inverse by walking the reconstructed forward hash. The inverse is a runtime cache, not wire format — consistent with the CVM rule that internal indexes don't need to survive the wire.
