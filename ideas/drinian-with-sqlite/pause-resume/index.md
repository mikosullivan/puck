...
The mechanism is a single stack frame. When code calls `%engine.pause`, the engine writes a "pause" frame at the top of the call stack in Drinian, then closes the database file. The process is gone. The state persists.

Resume is symmetric: any writer that can edit the Drinian file — an HTTP handler, an agent, a scheduled callback, a human tapping "approve" — removes the pause frame and optionally attaches a payload hash. When an engine reopens the database, execution picks up where it left off, and `%engine.pause` returns whatever payload the resumer wrote.
...
To resume a paused process, some other code opens the Drinian file and edits `call_stack`:
...
- **Agent coordination.** A supervisor agent pauses when it delegates a subtask. The subordinate agent completes the work, writes the result to the supervisor's Drinian file, resumes it. Multi-step agent flows without polling.
