...
Mark triggers are idempotent: setting `needs_trace = 1` on a row that's already marked is a harmless no-op. Each trigger in the schema is annotated with a `[set-needs-trace]` comment prefix so `grep [set-needs-trace] src/drinian.sql` enumerates the whole inventory.
...
- **`locals.value_object_pk`.** `after delete` and `after update of value_object_pk` — mark `old.value_object_pk`. Delete-time fires on frame pop via cascade; update-time fires on `$foo = something_else`.
- **`frame_ambers.amber_pk`.** `after delete` and `after update of amber_pk` — mark `old.amber_pk`. Delete-time fires on frame pop via cascade; update-time fires when a frame's domain gets rebound to a different amber instance. Amber instances themselves are ordinary objects; this bridge is where the reachability edge lives.
...
- **`frames.method_pk` / `method_class_pk` / `exception_class_pk`.** These live directly on `frames` and get two coverage paths: `after delete on frames` fires one composite trigger that marks all three OLD pointers at once (frame pop is the common case, so batching is worth it), and each column has its own `after update of <col>` trigger for in-place repointing.
