...
This doc describes Markie as the public-facing single-document service at `markie.uno`. Markie's underlying transformer engine is also used internally by other Puck services — most directly by [Donnie](https://puck.uno/ideas/dogberry/donnie), Dogberry's site-hosting service, which invokes the engine on every page request without going through the markie.uno HTTP API. Where this doc says "Markie," it usually means the public service; the engine is implementation detail shared across consumers.
...
Same model as [Gitter's language coverage](https://puck.uno/ideas/github/puck-site/gitter#language-coverage): **native** parsers for the languages Puck owns end-to-end (Caspian, JSON), **off-the-shelf wrapping** for everything mature community highlighters cover (Pygments family — Python, Ruby, JavaScript, TypeScript, HTML, CSS, Bash, SQL, etc.). Markie and Gitter share the same highlighting backend so the two never drift.
...
Submit a base layout with placeholder slots plus a derived fragment that fills them; Markie returns the assembled document. Same shape as Robinson's [target/content cascade](https://puck.uno/ideas/robinson/robinson#targetcontent-cascade), but offered to any client without requiring Robinson to be in the pipeline.

This is the per-call form. For the site-wide form (one base layout registered once, every page in a domain inherits it), see [Donnie's site-wide template inheritance](https://puck.uno/ideas/dogberry/donnie#site-wide-template-inheritance).
...
[Donnie's shared partials](https://puck.uno/ideas/dogberry/donnie#shared-partials).
