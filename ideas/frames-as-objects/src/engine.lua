...
CVM's per-connection rules — and wraps it as an engine instance.

Every prepared statement the engine will use gets prepped right here.
The block below is the engine's complete SQL manifest — one grep away
...
After `engine.new` returns, every `self.stmt_<name>` field is
