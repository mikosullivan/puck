...
- Keys under `coverage` are the file's identity — a local path for source in the user's tree, a URL for anything downloaded via [`%fetch`](https://puck.uno/requirements/chain/methods/puck).
