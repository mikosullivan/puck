-- Tests for db:add_hash() — inserts one hsa row with type='h', st=NULL, value=NULL.
...
h.test("add_hash returns a number for the new hsa_pk", function()
...
h.test("add_hash inserts row with type='h' and null st/value", function()
	local db = fiona.get_db(":memory:", "rw")
	local pk = db:add_hash()
	local row = row_for(db, pk)
	h.assert_eq(row.type, "h", "hsa.type is 'h' (hash)")
	h.assert_true(row.st == nil, "st is NULL for hashes")
	h.assert_true(row.value == nil, "value is NULL for hashes")
end)
