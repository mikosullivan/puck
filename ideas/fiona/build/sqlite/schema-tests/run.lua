#!/usr/bin/env lua5.4
-- Test runner for the Fiona database tests. Discovers every test_*.lua
-- file next to this runner and executes it in order; each file registers
-- its tests via helpers.test(). After all files run, reports totals and
...
for _, file in ipairs(files) do
	...
	print(string.format("%-40s %d passed, %d failed",
		file, h.results.passed, h.results.failed))
