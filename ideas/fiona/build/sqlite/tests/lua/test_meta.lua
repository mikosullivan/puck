...
-- {[key] = value}. Fresh DB has exactly one row: schema = '1'.
...
h.test("db:meta().schema is '1.0' on a fresh DB", function()
	local db = fiona.get_db(":memory:", "rw")
	h.assert_eq(db:meta().schema, "1.0", "schema row on fresh DB")
end)
...
h.test("db:meta() reflects new rows inserted into the meta table", function()
	...
	local m = db:meta()
	h.assert_eq(m.schema, "1.0", "schema still present")
	h.assert_eq(m.note, "hello", "new row shows up")
end)

h.test("db:meta() reflects updated values", function()
	local db = fiona.get_db(":memory:", "rw")
	db._conn:exec("update meta set value = '2.0' where key = 'schema'")
	h.assert_eq(db:meta().schema, "2.0", "updated value visible")
end)
...
h.test("db:meta() works on a read-only handle", function()
	...
	h.assert_eq(r:meta().schema, "1.0", "meta readable in 'r' mode")
