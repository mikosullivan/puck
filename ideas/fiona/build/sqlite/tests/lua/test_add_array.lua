-- Tests for db:add_array() — inserts one hsa row with type='a', st=NULL, value=NULL.
...
h.test("add_array returns a number for the new hsa_pk", function()
...
h.test("add_array inserts row with type='a' and null st/value", function()
	local db = fiona.get_db(":memory:", "rw")
	local pk = db:add_array()
	local row = row_for(db, pk)
	h.assert_eq(row.type, "a", "hsa.type is 'a' (array)")
	h.assert_true(row.st == nil, "st is NULL for arrays")
	h.assert_true(row.value == nil, "value is NULL for arrays")
end)
