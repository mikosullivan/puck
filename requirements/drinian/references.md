...
**Platter IDs are different**: they're UUIDs, not from this counter. The reason: platter IDs appear as keys inside user buckets (per the object-structure serialization), where integer-strings could collide with user-chosen field names. Object IDs don't have this exposure — they only appear in references, frame locals, and the objects-hash keys, never as markers inside user-controlled bucket data.
