...
- **Frame 1 — `array.each` running in `engine`.** This is a cross-role call: user code called into engine-owned built-in code. The frame records its iterator state (`{position: 0, of: 2}`), and carries its own `chain` (per [chain § Role boundaries reset everything](https://puck.uno/requirements/chain/#role-boundaries-reset-everything) — role boundaries reset the ambient chain slot). `role: engine`.
...
- **Chain is per-frame, with inheritance and role-boundary resets.** Per [chain § Frame inheritance](https://puck.uno/requirements/chain/#frame-inheritance), each frame gets its own chain; changes propagate DOWNWARD only (descendants see the changes, the parent sees its chain exactly as it was before the call). Role boundaries reset the chain: default-deny capabilities drop, the ambient hash slot empties, the callee starts with only default-granted surfaces. <!-- SPEC CONFLICT: archive says every frame gets a fresh chain on push regardless of role — no cascade, no aliasing, caller's values never visible to callee. Current chain spec (chain/index.md) says the OPPOSITE: chain inherits DOWNWARD within a role (descendants see ancestor values) and only resets at role boundaries. Rewritten to the current-spec model. Needs Miko decision if the archive's fresh-per-frame model was the intended one — the two are incompatible and change the shape of what `chain` looks like on the frame -->
