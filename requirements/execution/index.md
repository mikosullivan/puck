...
Execution is the operational phase — everything that happens after [bootstrap](https://www.puck.uno/requirements/bootstrap/) hands over a fully-prepared engine. Bootstrap gets the MVM open, seeded, and loaded with a CaspM tree; execution walks that tree and dispatches. The first user statement runs; the last user statement returns; anything in between is execution.
...
- After bootstrap returns, the MVM holds a loaded program (CaspM tree) alongside a seeded runtime state store.
...
- **First-dispatch mechanics** — how the engine transitions from "MVM has CaspM" to "the first statement executed."
