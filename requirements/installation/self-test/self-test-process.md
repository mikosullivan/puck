...
~~~vibecode
{"vibecode": {
	"doc": "requirements_installation_self_test_process",
	"role": "spec for how `caspian --self-test` loads Bryton (via %fetch) and downloads the test-tree tarball (via %net, extracted into a %tmp directory) — including the bryton.root marker at the archive's top level so Bryton recognizes the extract as a test root. Scope is deliberately limited to fetch mechanics — running the tests, rendering results, and handling failures belong to Bryton and the self-test renderer and are spec'd elsewhere. User-facing behavior lives in the sibling index.md.",
	"status": "spec — high-level pipeline enumerated; Bryton fetch defers to %fetch (not re-spec'd here); test-tree fetch is a plain %net tar.gz download extracted natively by the caspian binary into %tmp; downstream stages (run, render, report) spec'd elsewhere. Exact tarball URL scheme TBD.",
...
1. **Fetch** — load Bryton via [`%fetch`](../../fetch), and download the test-tree tarball via `%net` into a fresh `%tmp` directory.
...
- **Bryton** — loaded via [`%fetch`](../../fetch) in the usual way.
- **The test tree** — downloaded as a **tar.gz archive** from a URL on `caspian.uno` (exact scheme TBD — probably under `/download/`) via `%net` and extracted into a fresh `%tmp` directory. The archive's top-level directory contains a [`bryton.root`](../../bryton/runner/#the-testing-root) marker file so Bryton recognizes the extract as a test root.
