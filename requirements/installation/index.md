...
- **Pre-installed Lua libraries** — the Cache-tier set fetched during install and extracted to `~/.local/share/caspian/lua/`: `luaexpat` (the `lxp.so` binding to `libexpat.so.1` plus the Lua-side helpers `lom.lua` / `totable.lua` / `threat.lua`), `lsqlite3`, `lua-confstr`. See [core](../core/) for sizes and roles. Loaded lazily by `require`. `luaexpat` and `lsqlite3` both include a native `.so` that's per CPU architecture; the installer fetches the right one for the target platform through the same dispatcher pattern used for the `caspian` binary — `install.sh` forwards the discovered `arch=` to `caspian.uno/download/arch.casp`, which redirects to the matching build.
