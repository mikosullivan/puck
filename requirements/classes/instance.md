...
The **`instance`** keyword builds a single object directly, without going through a separate class declaration. The body uses the same shape as `class ... end` — fields, methods, inheritance — and everything in the body populates the [shadow class](https://puck.uno/requirements/concepts#classes-are-the-only-method-carrier) of the new object.
...
The args land in `&init` like any normal method call. The example uses the `@param` bucket-binding shorthand — `init(@host, @port)` stores each parameter directly into the bucket — but `init` is otherwise a regular method and any parameter shape it accepts works (positional, keyword, defaults, lazy, etc.). Full parameter mechanics live on [functions/bare § Parameters](../functions/bare#parameters).
...
If an `instance` body declares a method literally named `autorun`, the runtime invokes it after `init` and **the method's return value is what `instance` produces** — the constructed object itself is discarded.
...
`$dsn` above holds the string `'tcp://localhost:8080'`, not the constructed object. The build-and-use flow collapses into a single expression: `instance` constructs the object, calls its `.autorun` method, and hands back the result.

**Convention, not DSL.** No `autorun` keyword, no `.autorun` boolean property on methods, no symbol directive. If the method exists with that name, it runs; if not, `instance` returns the object as usual. Same pattern as `.call` making a class amp-invocable — the mechanism IS the naming convention.

**Instance-only.** The `autorun` convention matters inside `instance` bodies and has no counterpart in `class` construction. A class doesn't run on its own — it produces instances, and each caller of `.new` decides what to do with the result. A class body that defines an `autorun` method is just a class with a method named `autorun`; callers of `.new` get the constructed object and can invoke `.autorun` themselves if they want.
...
**When it doesn't fit.** If code outside the `instance ... end` block needs to reach back into the object, or if the object will be used more than once, don't declare an `autorun` method — construct the instance normally and drive it from the outside.

**Signature.** The `autorun` method takes no positional arguments — the runtime invokes it with an empty arg list. Arguments the caller passed to `instance` go to `&init`, not to `autorun`.
...
**With `autorun`.** When the body defines a method named `autorun`, the desugar has one extra step: after `.new`, `.autorun` is invoked and its return value becomes the value of the whole expression.
