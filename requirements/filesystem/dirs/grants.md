...
- [dirs § .cd()](https://puck.uno/requirements/filesystem/dirs/#cd) — one of the current nanny methods this spec grants.
- [linux-support § Executing from a directory](https://puck.uno/requirements/linux-support/#executing-from-a-directory) — `.execute_in`, the other current nanny method.
- [%fs](https://puck.uno/requirements/global-methods/fs) — where `.dirjail(...)` (which grants extend) is spec'd.
- [concepts § Security](https://puck.uno/requirements/concepts#security) — the holding-is-access rule that grants slot into.
