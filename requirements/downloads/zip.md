...
Stub. First-party download at `caspian.uno/linux/zip.casp` — read and write zip archives. Wraps the standard `zip` / `unzip` Linux utilities, matching the shellout style used by [tar](https://puck.uno/requirements/linux/cli/tar).
...
- [Tar](https://puck.uno/requirements/linux/cli/tar) — same shellout-wrapper pattern.
