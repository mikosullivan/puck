...
- **[Faucets](https://puck.uno/requirements/plumbing/faucets/)** bring values IN. Every inbound value (stdin bytes, an env var, a network response, a `%import` object) comes through a faucet with its own role, and the value carries that role.
...
- **Dual surfaces.** Several `%chain.X` methods are BOTH — `%chain.net` is a faucet (responses in) and a sink (requests out); `%fs` is both. Each side is governed by its own model; the object is one identity but plays both roles. <!-- STALE: %chain.X syntax being reworked -->
