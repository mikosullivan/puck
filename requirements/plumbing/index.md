...
- **Dual surfaces.** Several `%X` globals are BOTH — `%net` is a faucet (responses in) and a sink (requests out); `%fs` is both. Each side is governed by its own model; the object is one identity but plays both roles.
