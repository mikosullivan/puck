...
The CBOR decoder is the only genuinely new dependency. It is a **V1 download requirement** — fetched via `%(caspian.uno/cbor.casp)` on the first passkey call, then cached locally like any other `%import` object. The specific implementation (pure-Caspian decoder, C binding, or something else) is deferred; the requirement V1 commits to is that passkey code reaches it through the puck.uno URL, not that a specific library ships in the core install. Zero install-download cost; small first-use fetch when a program actually touches passkeys.
