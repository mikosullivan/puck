...
`processes` is a persistent table — rows survive across engine restarts. That's what enables pause / resume: when an engine reopens a Drinian file, it can either allocate a fresh `processes` row (new run) or look up the pk of a previously-suspended process and revive it.
