...
Returns the receiver's identity as a **string**. Assigned by Drinian at construction; immutable for the object's lifetime; treat the value as opaque. The format is Drinian's concern — a caller that wants to compare identities uses `==` on the agent or on the strings, and never parses or interprets the value.
