...
- **`null.obj.truthy?` returns `false`** — the truthy bit on a Null instance is `false`.
- **`null == null` is `true`** — two null values compare equal.
- **`null != false`** — null is not equal to false, though both are falsy.
- **`null` and `false` are distinct classes** — `null.obj.isa?(Boolean)` is `false`; `false.obj.isa?(Null)` is `false`.
...
- **Flavored null is still a Null instance** — after attaching any flavor value to a null, `.obj.isa?(Null)` remains `true`.
- **Flavored null is still falsy** — attaching a flavor does not flip the truthy bit; `.obj.truthy?` remains `false`.
...
- **`null.obj.isa?(Object)` is `true`** — Null ultimately extends Object.
