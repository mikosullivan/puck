...
- [Number](https://puck.uno/requirements/built-in-classes/primitives/number/) — the parent class; `.bitwise` is a method on every whole-value number.
- The [number § Predicates](https://puck.uno/requirements/built-in-classes/primitives/number/#predicates) — `.integer?`, `.even?`, `.odd?` — often paired with bitwise operations.
