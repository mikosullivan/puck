...
`%role.delegate_to($role) do ... end` extends the current frame's capabilities to a target role for the block. See the full spec in [roles § Granting capabilities to other roles](https://puck.uno/requirements/roles/#granting-capabilities-to-other-roles).
...
- **Default-granted capabilities cross role boundaries without help** — `%chain.puck`, `%chain.encryption`, etc. remain reachable across boundaries without any explicit grant.
