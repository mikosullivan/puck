...
Caspian doesn't have a "library" concept as a technical primitive — `%fetch` downloads objects (classes, instances, records, etc.). See [concepts § Objects, not libraries](https://puck.uno/requirements/concepts#objects-not-libraries) for the framing.
