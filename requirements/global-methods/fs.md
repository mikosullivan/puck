...
Values read through a nested dirjail carry the **original engine-provided dirjail's** role, not a role belonging to the nested view. A file read from `$bar_jail` is owned by `%fs`'s role, same as if it had been read directly through `$root`. This is the [narrowed-faucets rule](https://puck.uno/requirements/plumbing/faucets/#narrowed-faucets-dont-add-roles) applied concretely.
