...
local function respond(client, request_line)
    ...
    if r.kind == "sql_annotated" then
        -- ?raw=1 bypasses the annotator; same shape as lua_annotated.
        if query and query:find("raw=1", 1, true) then
            local data, read_err = read_file(r.path)
            if not data then
                client:send(build_response("500 Internal Server Error",
                    "Failed to read SQL file: " .. tostring(read_err) .. "\n",
                    "text/plain; charset=utf-8"))
                return
            end
            client:send(build_response("200 OK", data, "text/plain; charset=utf-8"))
        ...
        local title = r.path:match("([^/]+)$") or r.path
        local html = page.render_sql_annotated({
            path      = r.path,
            title     = title,
            client_ip = client_ip,
        })
