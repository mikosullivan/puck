...
runner.test("tag redirect: known tag /tag/grants returns redirect", function()
    local r = route.resolve("/tag/grants")
    assert_.equal(r.kind, "redirect")
    assert_.equal(r.location, "/documentation/requirements/caspian/filesystem/dirs/grants")
end)

runner.test("tag redirect: unknown tag /tag/no-such-tag returns not_found", function()
    local r = route.resolve("/tag/no-such-tag")
    assert_.equal(r.kind, "not_found")
end)

runner.test("tag redirect: bare /tag/ (no name) is not_found", function()
    local r = route.resolve("/tag/")
    assert_.equal(r.kind, "not_found")
end)
