...
h.test('schema seeds the user row', function()
	...
	for row in db:nrows("select count(*) as n from objects where role_core = 'u'") do
		count = row.n
	end
...
h.test('user row has persistent = 1', function()
	...
	for row in db:nrows("select persistent from objects where role_core = 'u'") do
...
h.test('engine row has role_parent = null (root of the core-role tree)', function()
	...
	for row in db:nrows("select role_parent from objects where role_core = 'e'") do
...
h.test('cache and user rows have role_parent set to engine', function()
	...
	local engine_pk
	for row in db:nrows("select object_pk from objects where role_core = 'e'") do
		engine_pk = row.object_pk
	end

	for _, role in ipairs({'c', 'u'}) do
		local role_parent
		for row in db:nrows(string.format("select role_parent from objects where role_core = '%s'", role)) do
			role_parent = row.role_parent
		end
...
h.test('cannot delete the user row (root role trigger)', function()
	...
	local rc = db:exec("delete from objects where role_core = 'u';")
	local msg = db:errmsg()
...
h.test("cannot insert a second row with role_core = 'u' (unique)", function()
	local db = cvm.open()

	-- Second user-role insert must satisfy other invariants first
	-- (owner_role required on non-roles; role_parent must exist; core
	-- roles must be pinned with persistent=1). Set them all, then let
	-- the UNIQUE fire on role_core.
	local rc = db:exec([[
		insert into objects (base, control, role_core, role_parent, owner_role, persistent) values ('o', 'r', 'u',
	...
	h.assert_true(rc ~= 0, 'second role_core=u insert should have failed but rc = ' .. tostring(rc))
...
h.test('cannot insert a role_parent pointing at a nonexistent row', function()
	...
	local rc = db:exec("insert into objects (base, control, role_parent) values ('o', 'r', '00000000-0000-4000-8000-000000000000');")
	local msg = db:errmsg()
...
h.test('cannot insert with role_parent pointing at a non-role row', function()
	...
	local stmt = db:prepare("insert into objects (base, owner_role) values ('h', ?);")
	...
	for row in db:nrows("select object_pk from objects where role_core = 'u' is null and role_parent is null and base = 'h' order by rowid desc limit 1") do
	...
	stmt = db:prepare("insert into objects (base, control, role_parent) values ('o', 'r', ?);")
...
h.test('can insert with role_parent pointing at a role (root or non-root)', function()
	...
	local stmt = db:prepare("insert into objects (base, control, role_parent) values ('o', 'r', ?);")
	stmt:bind_values(user_pk)
	stmt:step()
	stmt:finalize()
...
h.test('cannot update role_parent on an existing row (immutable)', function()
	...
	local rc = db:exec('update objects set role_parent = null where role_parent is not null and role_core is null;')
	local msg = db:errmsg()
...
h.test('non-role insert without owner_role raises objects_owner_role_required', function()
	...
	local rc = db:exec("insert into objects (base) values ('h');")
	local msg = db:errmsg()
...
h.test('role insert with owner_role is allowed (XOR rule dropped)', function()
	...
	local stmt = db:prepare(
		"insert into objects (base, control, role_parent, owner_role) values ('o', 'r', ?, ?);"
	)
...
h.test('owner_role pointing at a non-role raises owner_role_must_be_role', function()
	...
	local non_role_pk
	for row in db:nrows(
			"select object_pk from objects where role_core = 'u' is null and role_parent is null "
			.. "and base = 'h' order by rowid desc limit 1") do
		non_role_pk = row.object_pk
	end

	-- Now try to insert a non-role owned by that non-role.
	stmt = db:prepare("insert into objects (base, owner_role) values ('h', ?);")
	stmt:bind_values(non_role_pk)
	local rc = stmt:step()
	local msg = db:errmsg()
	stmt:finalize()
...
h.test('owner_role is immutable — update raises objects_owner_role_immutable', function()
	...
	local stmt = db:prepare(
		"insert into objects (base, control, role_parent) values ('o', 'r', ?);"
	)
	stmt:bind_values(user_pk)
	stmt:step()
	stmt:finalize()
	...
	-- core-role guard doesn't fire first.
	local rc = db:exec("update objects set owner_role = '" .. role_pk .. "' where owner_role is not null and role_core is null;")
	local msg = db:errmsg()
...
h.test('role_parent = object_pk raises objects_parent_role_not_self', function()
	local db = cvm.open()
	...
	local rc = db:exec(
		"insert into objects (object_pk, base, control, role_parent) values ('"
		.. self_pk .. "', 'o', 'r', '" .. self_pk .. "');"
	)
	local msg = db:errmsg()
...
h.test('owner_role = object_pk raises objects_owner_role_not_self', function()
	...
	local rc = db:exec(
		"insert into objects (object_pk, base, control, owner_role) values ('"
		.. self_pk .. "', 'o', 'r', '" .. self_pk .. "');"
	)
	local msg = db:errmsg()
...
h.test("engine seed has role_parent and owner_role both null (root of the core-role tree)", function()
	...
	local rp, ow
	for row in db:nrows("select role_parent, owner_role from objects where role_core = 'e'") do
		rp = row.role_parent
		ow = row.owner_role
	end
...
h.test('opening an already-installed DB is idempotent (skips the install)', function()
	...
	for row in db1:nrows("select object_pk from objects where role_core = 'u'") do
	...
	for row in db2:nrows("select count(*) as n from objects where role_core = 'u'") do
		count = row.n
	end
	...
	for row in db2:nrows("select object_pk from objects where role_core = 'u'") do
