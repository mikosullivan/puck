...
- **Unknown Content-Type falls through to non-Caspian handling** — a response with a Content-Type not on Caspian's own list is routed through the parser registry (see [non-caspian-mime-types](https://puck.uno/requirements/non-caspian-mime-types)).
...
- **Non-UTF-8 charset is transcoded** — a `text/x-caspian; charset=utf-16` response is decoded to UTF-8 before parsing (per [concepts § Strings are UTF-8](https://puck.uno/requirements/concepts#strings-are-utf-8)).
