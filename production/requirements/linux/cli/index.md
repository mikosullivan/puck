...
- **Never shells out.** `.execute` runs the utility directly (no `sh -c`); the wrapper composes argv arrays, not command strings. See [linux-support § Executing files](https://puck.uno/requirements/linux-support/#executing-files) for why.
- **Is user-role only.** `.execute` is user-only in V1 (see [linux-support § Executing from a directory](https://puck.uno/requirements/linux-support/#executing-from-a-directory)); wrappers inherit that restriction. Non-user roles can't invoke a wrapper at all, granted or not.
...
- [openssl](openssl) — ES256 / RS256 signature verify for the [Passkey subsystem](https://puck.uno/requirements/protected/passkey/). Predates the priority list; migrated in from `linux-support/` as the namespace consolidated.

## Prior-art notes

Two wrapper stubs also exist under [downloads/](https://puck.uno/requirements/downloads/) for [gzip](https://puck.uno/requirements/downloads/gzip) and [zip](https://puck.uno/requirements/downloads/zip); those flagged an unresolved Ships-tier decision (core vs download). This dir is where the per-wrapper specs live regardless of the eventual Ships tier.
