...
If mutating String is undesirable, apply the class's methods explicitly via the [downloaded-method](https://puck.uno/requirements/classes/downloaded-methods) form. Nothing global changes; each call opts in individually:
...
- **Array's block-form `.ensure`** — `.inherited` uses it, but the general Array-level `.ensure` (bare and block, on any array) still needs its own dedicated spec entry under [primitives/array/](https://puck.uno/requirements/built-in-classes/primitives/array/). Behavior mirrors what's shown on [classes/inheritance](tag:class-inheritance).
