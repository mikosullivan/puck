...
- **`Closure`** — the frame for a closure's body. Created fresh on each invocation. Also the frame for an `if` / `unless` branch body and for a `while` / `until` / `.each` / `.times` loop body — those are closures per [functions/closure](https://www.puck.uno/requirements/functions/closure).
