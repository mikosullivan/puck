...
- **Number literal materializes without `%fetch`** — evaluating `42` in an engine with the network disabled produces a Number instance and does not attempt any `%fetch` call.
...
- **`42.obj.isa?(Number)`** is `true`.
- **`'hi'.obj.isa?(String)`** is `true`.
- **`true.obj.isa?(Boolean)`** is `true`.
- **`false.obj.isa?(Boolean)`** is `true`.
- **`null.obj.isa?(Null)`** is `true`.
- **`[1, 2, 3].obj.isa?(Array)`** is `true`.
- **`{a: 1}.obj.isa?(Hash)`** is `true`.
- **Object class is also guaranteed at startup** — `%('puck.uno/object')` returns a class value in a fresh runtime.
- **Every value is an Object** — for each primitive literal above, `.obj.isa?(Object)` is `true`.
