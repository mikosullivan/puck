...
`false` is one of the two **falsy** values (the other is `null`). Every other value in Caspian — including `0`, `''`, `[]`, `{}`, and any user-defined object — is truthy. Full rules at [syntax § Truthy and falsy](https://puck.uno/requirements/syntax/truthy-and-falsy).
...
- [Syntax § Truthy and falsy](https://puck.uno/requirements/syntax/truthy-and-falsy) — how booleans compose with Caspian's truthy/falsy rule.
- [Syntax § Operators](https://puck.uno/requirements/syntax/operators) — the logical operators that dispatch to boolean methods.
