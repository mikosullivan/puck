...
~~~vibecode
{"vibecode": {
	"doc": "requirements_lua",
	"role": "index for requirements/lua/ — Caspian's Lua-ecosystem integration beyond what ships in the core install. Currently covers third-party Lua library installation (both pure Lua and C bindings) via `caspian --install-lua <name>`, which wraps a user-supplied luarocks. See ../core/ for the caspian binary and pre-installed Lua libraries.",
