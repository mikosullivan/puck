...
{"doc": "requirements_cvm_ownership",
	"role": "Ownership of a bucket or stack is a normal refs row from the owner to the collection — no dedicated bucket_pk / stack_pk / bucket_for / stack_for columns. Non-container parents (base = 'o') are capped at one hash-child (their bucket) and one array-child (their stack). Sharing across owners falls out. Cascade cleanup runs through the standard refs_mark_needs_trace_after_delete trigger — no bespoke per-collection triggers.",
...
A non-container parent — `base = 'o'` (which covers plain objects, frames, and roles alike, since `control` layers on top of `'o'`) — can hold **at most one HashPrimitive child (its bucket) and at most one ArrayPrimitive child (its stack)** as refs. Container parents (`base = 'h'` or `'a'`) have no such cap: hashes and arrays hold as many refs as they need by their native semantics.
...
- `key` — `null` (the child's `base` tells you which is bucket vs stack)
- `idx` — auto-assigned, following the `add_ref` conventions

The child's `base` field disambiguates: `base = 'h'` → the owner's bucket; `base = 'a'` → the owner's stack.
...
Both use correlated subqueries filtered by `child.base` — guaranteed to return at most one row per owner by the one-hash-one-array cap.

## What this replaces

Earlier drafts carried dedicated `bucket_pk` / `stack_pk` columns on the owner and back-reference `bucket_for` / `stack_for` columns on the collection, with per-column immutability, target-`base` checks, and needs_trace triggers. All of that is gone — the refs table plus the one cap trigger cover it uniformly.
