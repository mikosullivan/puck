...
The variable-scalar handler writes a binding into scope[0] inline, wrapped in a savepoint. Four CVM calls: `cvm:add_scalar(value, role_pk)` materializes the scalar (polymorphic on the Lua-type of `value` — routes to `scalar_string` / `scalar_number` / `scalar_bool` / `scalar_null`); `cvm:ensure_own_scope(frame_pk, role_pk)` ensures the bucket → scopes → scopes[0] chain and returns the own-scope pk; `cvm:upsert_ref(own_scope_pk, name, scalar_pk)` binds `name` to the scalar; `cvm:mark_frame_gc(frame_pk)` sets `frame_gc = 1` so the walker's advance-preconditions are met. See [frame-lifecycle](./frame-lifecycle) for how the enclosing walker advance completes the cycle.
