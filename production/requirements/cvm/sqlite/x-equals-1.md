~~~vibecode
{"vibecode": {
	"doc": "requirements_cvm_x_equals_1_walkthrough",
	"role": "Concrete illustration of the schema in action. Runs `$x = 1` through the engine (with a `%process.stop` afterward so the graph stays visible), dumps the full state of `objects` and `refs`, and walks through each row / each ref. Load-bearing detail: the scalar `1` lands as `scalar_number = 1.0` — REAL affinity coerces the Lua integer input to a float on insert — while `scalar_null` / `scalar_string` / `scalar_bool` all stay null. One value shape for every number Caspian holds.",
	"audience": "developers reading the schema doc who want to see a concrete run rather than abstract-shape prose; anyone learning what happens when Caspian dispatches even the simplest statement"
}}
~~~
