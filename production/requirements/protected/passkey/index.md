...
| `openssl` subprocess (ES256, RS256) | new — via the [linux/cli/openssl](https://puck.uno/requirements/linux/cli/openssl) wrapper class; operator-provided | ES256 / RS256 assertion verify. |
...
The CBOR decoder is the only genuinely new dependency. It is a **V1 download requirement** — fetched via `%(caspian.uno/cbor.casp)` on the first passkey call, then held in the local byte cache like any other `%fetch` target. The specific implementation (pure-Caspian decoder, C binding, or something else) is deferred; the requirement V1 commits to is that passkey code reaches it through the puck.uno URL, not that a specific library ships in the core install. Zero install-download cost; small first-use fetch when a program actually touches passkeys.
...
**ES256 / RS256 signature verify** calls the operator-installed [`openssl`](https://puck.uno/requirements/linux/cli/openssl) binary directly via `.execute` — subprocess invocation, no shell involved (see [linux-support § Executing files](https://puck.uno/requirements/linux-support/#executing-files)). Zero bundled bytes; treated as a Caspian prerequisite in the same posture as `luarocks` (third-party Lua libs) and `tar` (self-test tarball extraction). Widely available on every mainstream Linux distro and on macOS. Documented in [linux/cli/openssl](https://puck.uno/requirements/linux/cli/openssl).
...
- [linux/cli/openssl](https://puck.uno/requirements/linux/cli/openssl) — the wrapper class that invokes the operator-installed `openssl` binary directly for ES256 / RS256 signature verify.
