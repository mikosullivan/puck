...
- **Nested named definition is fresh per outer-invocation** — `function &outer() function &inner() end; return %module.methods['inner'] end; %module.outer().obj.pk != %module.outer().obj.pk`: two calls to outer produce two distinct inner function objects.
- **Top-level function's `%module` is the persistent Script frame** — `function &foo() return %module.obj.pk end; %module.foo == %module.foo`: two calls return the same frame id.
- **Nested function's `%module` is a per-invocation frame** — `function &outer() function &inner() return %module.obj.pk end; return %module.methods['inner'] end; %module.outer().call != %module.outer().call`: two outer-invocations produce two frames.
