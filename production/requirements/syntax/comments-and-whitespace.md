...
Newline-non-significance is a **general policy**, not an absolute rule. Specific constructs may require newlines to mark structure — [heredocs](https://puck.uno/requirements/built-in-classes/primitives/string/heredocs) are one such case, where the terminator label must appear on its own line. Additional exceptions may emerge as language features are spec'd; each is called out on the page that owns the construct.
...
Both are **reserved BWCs** — parse-recognized barewords in the same class as `field`, `private`, `inherits`. User code cannot rebind them; there is no `$documentation` variable or method-of-same-name that can shadow the construct. They are neither system-methods (`%name` sigil) nor user-definable functions.
...
Full spec at [global-methods § `documentation`](https://puck.uno/requirements/global-methods/#documentation) and [global-methods § `vibecode`](https://puck.uno/requirements/global-methods/#vibecode).
