...
<!-- STALE: %chain.X syntax being reworked — every claim below about %chain.X as a canonical form is out-of-date; %chain no longer carries methods. See [chain/index](https://puck.uno/archive/003/misc/chain-old/). -->
...
Other globals on `%chain` (`%net`, `%timer`, `%timeout`, etc.) have no bare shortcut and are only reachable through the canonical form.

For the catalog of every sigil, per-sigil semantics, and which bare shortcuts exist, see [global-methods](https://puck.uno/requirements/global-methods/). Chain-mediated methods have their canonical specs under [chain/methods/](https://puck.uno/archive/003/misc/chain-old/methods/).
...
- **`%chain.X` resolves the named chain-mediated capability** — `%puck` returns the object-download surface.
...
- **Bare `%fetch` shortcut resolves to `%puck`** — both yield the same value.
- **Bare `%stdin` shortcut resolves to `%stdin`** — both yield the same value.
- **Bare `%stdout` shortcut resolves to `%stdout`** — both yield the same value.
- **Bare `%stderr` shortcut resolves to `%stderr`** — both yield the same value.
- **`%net` has no bare shortcut** — `%net` as a standalone form raises (no such bare shortcut); `%net` works.
- **`%timer` has no bare shortcut** — bare `%timer` raises; `%timer` works.
- **`%timeout` has no bare shortcut** — bare `%timeout` raises; `%timeout` works.
- **`.grant` operates on canonical `%chain.X` form** — granting `%stdout` also grants the bare `%stdout`.
- **`.revoke` on `%chain.X` revokes the bare form** — after revoking `%stdout`, both `%stdout` and `%stdout` raise "not granted".
...
- **`%X.Y` chain access parses** — `%puck` and `%engine.something` both parse in the two-dot form.
