...
|---|---|---|
<!-- STALE: %chain.X syntax being reworked — the `%chain.X` references throughout this file (table rows below, testing bullets further down) predate the permission-only %chain model. See [chain/index](https://puck.uno/archive/003/misc/chain-old/). -->
| stdin | [`%stdin`](https://puck.uno/archive/003/misc/chain-old/methods/stdin) / `%engine.stdin` | Bytes piped to the process. |
| argv | [`%chain.argv`](https://puck.uno/archive/003/misc/chain-old/methods/argv) / `%engine.argv` | Command-line arguments. |
| env | [`%chain.env`](https://puck.uno/archive/003/misc/chain-old/methods/env) | Environment variables. |
| filesystem | [`%fs`](https://puck.uno/requirements/global-methods/fs) / `%tmp` | File contents and directory listings read through dirjails. |
| network | [`%net`](https://puck.uno/archive/003/misc/chain-old/methods/net) | HTTP response bodies, socket reads, UDS data. |
...
Some faucets can be **narrowed** — the caller creates a restricted view of a broader faucet without going back to the engine. Dirjails are the primary case: any code holding a dirjail can construct a nested dirjail rooted at a subdirectory (see [`%fs` § Nested dirjails](https://puck.uno/requirements/global-methods/fs#nested-dirjails)). The same pattern applies to other narrowable faucets — a per-host wrapper around `%net`, for instance.

**Values read through a narrowed faucet carry the parent faucet's role — not a per-narrowing role.** A nested dirjail rooted at `%fs.root['foo']` still delivers files whose owner is `%fs`'s role. A per-host wrapper around `%net` still delivers responses whose owner is `%net`'s role. Narrowing restricts *what can be reached*, but doesn't change *what the source is*.
...
- **Every faucet in the catalog has a distinct role** — comparing `%stdin.read.obj.role`, `%chain.argv[0].obj.role`, `%chain.env['X'].obj.role`, and `%net.fetch(url).body.obj.role` yields four different roles.
- **A stdin-read string is not `user`-owned** — `%stdin.read.obj.role != %role`.
...
- **A network response body is not `user`-owned** — `%net.fetch(url).body.obj.role != %role`.
...
- **A per-host net wrapper's response is owned by `%net`'s role** — the wrapping doesn't mint a new role.
...
- **Storing a faucet-owned value in a user-owned container preserves the value's role** — `$hash[k] = %stdin.read; $hash[k].obj.role` is stdin's role.
...
- **A file read via a jail on `%net` produces responses owned by the net faucet role** — jail doesn't launder.
- **`%puck` downloaded objects are owned by the download faucet role** — distinct from network-response role.
